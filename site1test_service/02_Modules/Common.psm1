...
function Show-ToolkitHeader {
    ...
    Write-Host '=== Site 1 Service Block Test Toolkit ===' -ForegroundColor Cyan
...
function Invoke-ToolkitWinrmTest {
    ...
    try {
        ...
        if ([string]::IsNullOrWhiteSpace($output)) {
          ...
          $status = Get-ToolkitStatus -Succeeded $true -Output $output -SuccessPatterns $SuccessPatterns -MissingEvidenceStatus $MissingEvidenceStatus
        if (-not $CheckDescription) {
...
function Invoke-ToolkitSshTest {
    ...
    try {
        ...
        if (-not $output) {
          ...
          $status = Get-ToolkitStatus -Succeeded ($client.IsConnected -and ($command.ExitStatus -eq 0)) -Output $output -SuccessPatterns $SuccessPatterns -MissingEvidenceStatus $MissingEvidenceStatus
        if (-not $CheckDescription) {
...
"@
        ...
        if (-not $CheckDescription) {
            $CheckDescription = "Request $Uri and confirm the web application returns a valid HTTP response."
        }
        if (-not $CommandLines -or $CommandLines.Count -eq 0) {
