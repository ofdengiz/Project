...
This test report adapts the Site 1 service block checklist format for Site 2 and records the final read-only validation state of the environment as of March 23, 2026, while also noting the subsequent live Veeam remediation findings that materially affect the backup conclusion. No configuration changes were made during the original read-only pass. The goal was to confirm that Site 2 core services, cross-site dependencies, and final demo paths were healthy enough to support handover and presentation.
...
- Site 2 Veeam management listeners exist, but the latest live troubleshooting pass showed unresolved control-path issues between protected hosts and `S2Veeam`.
- A small number of rows remain `REVIEW` because they need either GUI screenshots, interactive user-session proof, or final firewall-rule remediation rather than non-interactive read-only probing.
...
- Live Veeam job testing on March 23 still showed timeouts from protected Linux hosts to `172.30.65.180:10005` and `:10006`.
- OPNsense review showed that `S2_VEEAM` was not included in the normal `C1_GLOBAL` / `C2_GLOBAL` allow scope and that `VEEAM_COPY_PORTS` did not include `10005` or `10006`.
- Veeam policy testing also showed overlap between older classic jobs and newer Windows/Linux agent policies, producing `already processed by backup job` conflicts.
- The Veeam platform is therefore implemented but not yet final-signed-off; the remaining work is path and ownership remediation, not platform installation.
...
- backup host listeners present, but final protected-host path still under remediation

The remaining `REVIEW` items are now split between documentation-evidence gaps and one real remediation track: Veeam host-to-server pathing. Core Site 2 identity, DNS, HTTPS, and file-replication services remain healthy. The main open technical item is to finish the `S2Veeam` rule scope and policy cleanup so the live agent/file jobs can be signed off cleanly.
