...
The two Company 1 client roles also support different types of proof. `C1UbuntuClient` is valuable for showing that the documented hostname model works cleanly from the Company 1 perspective, especially for cross-site HTTPS consumption and shell-visible domain context. `C1WindowsClient`, by contrast, is the more natural endpoint for demonstrating the lived Public or Private share workflow in the exact form that a Company 1 user would experience it. Treating those two clients as complementary rather than interchangeable makes the Company 1 narrative stronger and more realistic.
