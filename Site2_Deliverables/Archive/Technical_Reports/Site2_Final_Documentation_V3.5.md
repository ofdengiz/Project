...
| Document Version | 3.4 Working Revision |
...
![Figure 1. `C2IdM1` final replication and DNS query proof from the approved admin path.](DocAssets/c2idm1-repl-dns.png)

![Figure 2. `C2IdM2` final replication and DNS query proof from the approved admin path.](DocAssets/c2idm2-repl-dns.png)
...
![Figure 3. `C2LinuxClient` hostname resolution proof for `c1-webserver.c1.local` and `c2-webserver.c2.local`.](DocAssets/c2linuxclient-dns.png)

![Figure 4. `C2LinuxClient` SSSD, realm, and domain-user lookup proof.](DocAssets/c2linuxclient-sssd.png)

![Figure 5. `C2WebServer` Docker status and hostname-only hardening proof showing raw IP `404` and hostname `200`.](DocAssets/c2web-hardening.png)
...
![Figure 6. `C2FS` mounted storage, published directories, and successful sync-log proof.](DocAssets/c2fs-storage-sync.png)
...
![Figure 7. Representative Veeam console evidence from Site 2 backup validation and troubleshooting.](DocAssets/veeam-success-history.png)

![Figure 8. Windows Explorer view of `R:\Repo_Site2_Offsite\Site2OffsiteFromSite2` showing backup-copy folders.](DocAssets/veeam-offsite-copy-folder.png)
...
![Figure 9. Public browser view of the ClearRoots Foundation landing page over HTTPS.](DocAssets/aws-public-site.png)

![Figure 10. `kubectl get nodes` and `kubectl get pods` showing both nodes Ready and the ClearRoots pods Running.](DocAssets/aws-k8s-nodes-pods.png)

![Figure 11. AWS EC2 console showing `clearroots-kube-master` and `clearroots-kube-worker` in the Running state.](DocAssets/aws-ec2-instances.png)

![Figure 12. Amazon S3 console showing the Terraform state object under `clearroots/k8s/terraform.tfstate`.](DocAssets/aws-s3-state.png)

![Figure 13. Route53 public hosted zone showing the A record for `clearroots.omerdengiz.com`.](DocAssets/aws-route53.png)
...
The March 22 read-only validation pass confirmed that the core Site 2 service blocks were healthy. The remaining gaps are documentation-evidence gaps rather than unresolved service failures. To keep the final package disciplined, the remaining screenshots should be collected in the same order that the document introduces its claims, with Veeam left to the end.

The highest-value remaining non-Veeam captures are:

- `Jump64` final DNS order, because Section `3.2` and Section `3.10.2` both rely on the corrected Windows jump workflow
- `mspubuntujump` resolver configuration, because Section `3.2`, Section `3.6`, and Section `3.10.2` all treat the Ubuntu jump as the most reliable validation vantage point
