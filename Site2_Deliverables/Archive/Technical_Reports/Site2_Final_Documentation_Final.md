...
subtitle: "Design and Implementation of a Site 2 Hybrid Infrastructure Using Proxmox Services, Samba Identity, Veeam Backup, and an AWS-Hosted HTTPS Web Platform"
author:
  - "Bailey Kulla"
  - "Elyazid Sidelkheir"
  - "Ru Wang"
...
- Tailscale to the Ubuntu jump for clean internal SSH, DNS, SMB, and HTTP validation
...
| Company 2 web | Apache HTTP response from `172.30.65.170` | Healthy | Internal HTTP path confirmed |
| Company 1 internal web | HTTP and HTTPS from Ubuntu jump | Healthy | Dual-stack internal publication path confirmed |
| Cross-site internal web failover path | `curl -k`, `nslookup`, and OpenVPN rule validation | Healthy | Site 1 and Site 2 could reach the dual-record internal web names after targeted `80/443` rule additions |
...
- C2Web recovered and both HTTP and HTTPS became reachable from approved validation paths.
