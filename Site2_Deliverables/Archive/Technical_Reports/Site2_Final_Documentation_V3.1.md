...
Operationally, the environment progressed through several troubleshooting stages before stabilization. Earlier testing identified issues with Windows jump routing behavior, DMZ web server reachability, C2LinuxClient SSSD identity resolution, and the Site 2 to Site 1 sync path. Those core service issues were later corrected. The current validated state shows healthy Company 2 identity services, restored web reachability, functioning Linux client identity lookup, successful file synchronization, and healthy Kubernetes web deployment. The Veeam design and repository layout are implemented, but the final agent/file backup path was still under active remediation at the document freeze point due to agent-policy overlap, `10005/10006` reachability, and OPNsense rule-scope gaps to `S2Veeam`.
...
Backup and recovery became one of the most technically important Site 2 workstreams, but it also remained the area with the most late-stage remediation. After an extended troubleshooting phase, the team rebuilt the Site 2 Veeam environment, validated service listeners and repository paths, and isolated the remaining blockers to connection management and firewall scope rather than storage loss.

The Site 2 Veeam implementation includes three layers:
...
- older classic agent jobs conflicted with newly created Windows and Linux agent policies, producing `already processed by backup job` errors
- OPNsense alias and rule coverage did not yet fully include `S2_VEEAM` from the `C1LAN` and `C2LAN` source interfaces, and the `VEEAM_COPY_PORTS` alias did not include `10005` or `10006`

These points matter because the failing jobs did not behave like a damaged repository. They behaved like a control-path problem between protected hosts, the backup server, and the routed rule set.

### 3.8.1 Agent-based system backup

Site 2 Veeam was configured to protect a mixed set of Windows and Linux systems through agent-based protection. During the final correction phase, that design was refactored toward operating-system-specific groupings so that Windows and Linux systems would not compete for the same policy or classic backup job path.
...
The live remediation pass later showed that keeping the same hosts in both old classic jobs and new agent policies caused avoidable conflicts. For the final handover, the correct operating model is:
...
This design is appropriate because file-share retention and restore workflows are different from full system recovery workflows. It also made the later copy jobs easier to interpret in the offsite repository, where separate copy folders were created automatically by Veeam. During the March 23 live troubleshooting pass, these file jobs were still failing when protected hosts attempted to establish backup-server roles against `veeam`, which confirmed that the remaining issue was name/path/control-plane related rather than a missing repository folder.
...
This confirms that the intended Site 2 design no longer protects data only locally. It now includes an additional recovery location in Site 1, which materially improves resilience against local repository loss or single-site failure. The offsite repository structure existed and historical copy evidence was available, but the latest live Veeam validation should still be treated as in-progress until the remaining agent and OPNsense path issues are closed.
...
|---|---|---|
| Windows_Servers / Windows agent policy set | Windows agent backup | Agent-based protection for Site 2 Windows hosts, grouped to avoid policy overlap |
| Ubuntu_Servers / Linux agent policy set | Linux agent backup | Agent-based protection for Site 2 Linux hosts, grouped to avoid policy overlap |
...
| Late agent remediation | Agent jobs timed out to `172.30.65.180:10005/10006` | Suggested Windows Firewall or routed-port problem | Listener checks on `S2Veeam` succeeded; inbound Windows allow rule was added, but external Linux tests still timed out |
| Policy conflict phase | New policies reported hosts already processed by other backup jobs | Suggested duplicate protection ownership | Old classic jobs and new policies were found to overlap and must be separated |
| OPNsense scope review | Site 2 protected hosts still could not reach `S2Veeam` management ports | Suggested network-rule mismatch rather than Veeam service failure | `S2_VEEAM` was not covered by `C1_GLOBAL` / `C2_GLOBAL`, and `VEEAM_COPY_PORTS` omitted `10005/10006`; final LAN/OpenVPN rule cleanup remained open |
...
![Figure 6. Representative Veeam console evidence from Site 2 backup validation and troubleshooting.](DocAssets/veeam-success-history.png)
...
| Veeam local jobs | Veeam console troubleshooting and port tests | In remediation | Backup design is implemented, but the latest live Windows/Linux/file jobs still showed agent-path and policy-overlap issues |
| Veeam offsite copy | SMB target folder and copy history | Implemented with historical evidence | Site 1 offsite folders and repository mapping exist, but the latest end-to-end live signoff remains tied to Veeam remediation |
...
| 5 | Veeam and miscellaneous support evidence | In remediation | `S2Veeam` listeners and repository objects exist, but final signoff still depends on resolving `10005/10006` pathing, policy overlap, and `S2_VEEAM` OPNsense rule scope |
...
| Backup operator | Local and offsite protection plus final remediation | Veeam job state, listener tests, offsite SMB repository folders, OPNsense rule coverage | Veeam console, Site 2 firewall GUI, and Site 1 offsite share |
...
- the backup platform is implemented and partially validated, but final operational signoff still depends on closing the remaining VBR path and policy-overlap issues
...
- Veeam now has a rebuilt local/offsite design with verified listeners, repositories, and troubleshooting evidence, but the final live backup signoff remains in remediation
...
|---|---|---|
| Windows agent-based protection set | Windows agent backup | In remediation |
| Linux agent-based protection set | Linux agent backup | In remediation |
| C1_FileShare | File backup | In remediation |
| C2_FileShare | File backup | In remediation |
| Historical backup-copy objects | Backup copy | Historical success evidence present |
| Site 2 offsite copy path | Backup copy | Repository and folder evidence present |
...
- Veeam console evidence demonstrates a fully implemented backup design and a narrowed final issue set rather than a missing platform.
- offsite repository folder evidence demonstrates that backup-copy traffic reached Site 1 at least during earlier successful validation phases.
