...
**Table 14.** Demo readiness and probable instructor scenarios  
**Table 15.** Operational maintenance checks  
**Table 16.** Current limitations and evidence gaps  
...
## 3.9 Demo Readiness and Probable Instructor Scenarios

The week 6 notes warn that demo week operates on a short timer and that important consoles should already be open. The best Site 2 demo is therefore not a random walk through servers. It is a prepared sequence aligned to the Service Blocks model and to the fastest available proof paths.
...
| Show segmentation and controlled exposure | OPNsense interfaces, NAT, aliases, and OpenVPN rules |
| Show that Company 2 identity is healthy | `C2IdM1` and `C2IdM2` service state plus DNS records |
| Show that Company 1 still exists in Site 2 scope | Company 1 DNS records on Company 2 identity services plus Company 1 web access |
| Show storage and SAN | `C2FS` iSCSI session, mount, share definitions, and SAN screenshots |
| Show that both companies can consume the web services | `S2-Ubuntu-Client` and `C2LinuxClient` access to both hostnames |
| Show backup readiness | `S2Veeam` ports plus Veeam console screenshot |
| Show supportability | Use the bastion path and explain how the design narrows edge exposure |

The practical takeaway is simple: keep the jump hosts, OPNsense view, identity services, client web proof, storage proof, and Veeam console ready before the demo starts.

## 3.10 Maintenance and Daily Duties

The Site 1 final report includes maintenance and daily duties, and the week 9 technical-report slides explicitly ask what maintenance matters. Site 2 therefore needs an operations section as well.
...
| Confirm Company 1 and Company 2 hostnames still return `200` | Prevents demo-day web surprises |

## 3.11 Limitations, Risks, and Remaining Evidence Gaps
...
These are acceptable limitations for a read-only validation pass. They do not weaken the strong evidence already collected for segmentation, identity, DNS, storage, client behavior, internal web delivery, and backup-path design.
