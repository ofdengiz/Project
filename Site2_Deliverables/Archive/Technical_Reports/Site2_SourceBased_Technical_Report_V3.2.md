...
<p>&nbsp;&nbsp;&nbsp;2.5 Delivery-phase change history and finalization decisions</p>
<p>3&#46; Discussion</p>
<p>&nbsp;&nbsp;&nbsp;3.1 Environment overview and service boundaries</p>
<p>&nbsp;&nbsp;&nbsp;3.2 Site 2 logical service inventory and platform roles</p>
<p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Observed Linux VM platform baseline</p>
<p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Service placement rationale</p>
<p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Technology and service-stack rationale</p>
<p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Delivery-phase configuration refinements</p>
<p>&nbsp;&nbsp;&nbsp;3.3 MSP entry, network segmentation, remote access, and security rationale</p>
<p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Interface and segment design</p>
<p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Policy and exposure model</p>
<p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Operational interpretation</p>
<p>&nbsp;&nbsp;&nbsp;3.4 Company 1 services</p>
<p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Service overview</p>
<p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Architectural rationale</p>
<p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Observed operating state</p>
<p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Service composition and operational reading</p>
<p>&nbsp;&nbsp;&nbsp;3.5 Company 2 identity, shared forest context, DNS, and DHCP</p>
<p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Identity service health</p>
<p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Namespace and forest design</p>
<p>&nbsp;&nbsp;&nbsp;3.6 Storage, file services, and isolated SAN design</p>
<p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;File-service state</p>
<p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;SAN isolation model</p>
<p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Share presentation model</p>
<p>&nbsp;&nbsp;&nbsp;3.7 Client access, identity validation, and dual-hostname web delivery</p>
<p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Client validation perspectives</p>
<p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Client resolver and share-consumption alignment</p>
<p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Hostname-based web publishing behavior</p>
<p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Client-service interpretation</p>
<p>&nbsp;&nbsp;&nbsp;3.8 Backup, recovery, and offsite protection</p>
<p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Backup design basis</p>
<p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Inter-site backup path</p>
<p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Delivery-phase backup and credential refinements</p>
<p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Current operational state</p>
<p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Recovery role in the overall site design</p>
<p>&nbsp;&nbsp;&nbsp;3.9 Requirement-to-implementation traceability</p>
<p>&nbsp;&nbsp;&nbsp;3.10 Service dependency, failure domains, and access model</p>
<p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Dependency view</p>
<p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Access model interpretation</p>
<p>&nbsp;&nbsp;&nbsp;3.11 Storage, backup, and recovery flow</p>
<p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Data-protection layers</p>
<p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Sync versus backup</p>
<p>&nbsp;&nbsp;&nbsp;3.12 Maintenance and daily duties</p>
<p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Routine operational checks</p>
<p>&nbsp;&nbsp;&nbsp;3.13 Troubleshooting and fast triage guide</p>
<p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Fast triage principles</p>
...
**Table 3A.** Delivery-phase change and finalization summary  
...
- hostname-based SMB validation from `C2LinuxClient` to `c2fs.c2.local` confirmed browse, public-share write or delete, and user-specific private-share access without relying on a client-side raw-IP shortcut
...
The external hostname is intentionally excluded from the technical narrative because it is outside the required scope of this package.
...
The evidence set establishes two things at the same time: what Site 2 is expected to deliver as an operational environment, and what can be confirmed directly from the current implementation.
...
| Current operating observations | March 23, March 24, and March 25, 2026 observations from the approved remote path, jump hosts, MSP bastion, and targeted CLI inspection | Describes current operating state and confirms the final deployed service behavior |
...
The observation approach followed a management-first bastion model. `MSPUbuntuJump` was treated as the primary bastion for targeted CLI inspection into Company 1 and Company 2 paths. The Windows jump and local workstation were used only where they added practical reachability context. Most CLI inspection against selected Linux systems was limited to state confirmation; where a current service behavior had drifted from the intended hostname-based design, the final operating state also included a narrowly scoped corrective update followed immediately by revalidation.
...
| `MSPUbuntuJump` | To perform targeted CLI inspection from the MSP management plane into Company 1 and Company 2 paths |
...
## 2.5 Delivery-Phase Change History and Finalization Decisions

The final Site 2 state was not produced by a single build pass. It emerged through controlled correction and revalidation across backup transport, web publication, client resolution behavior, and test-tool alignment. That delivery history matters because it explains why the final operating model is supportable: the environment was not only assembled, but also refined until the observed behavior matched the intended design.

Several decisions were especially important during finalization. In the backup layer, early Veeam symptoms pointed to a mixture of name-resolution issues, transport-path issues, and stale credential binding rather than to a single failed service. In the web layer, the final publication model was tightened so that Company 2 served only the intended HTTPS hostname path rather than remaining loosely reachable over HTTP or raw IP. In the client layer, `C2LinuxClient` was brought back to a true multi-namespace resolver posture so that `c1.local` and `c2.local` both worked through name resolution instead of through address-specific workarounds. In the validation layer, test-tool defaults were reconciled with the actual source-of-truth inventory so that the final evidence reflected the real environment rather than temporary assumptions.

This history is worth documenting because it shows which adjustments were corrective necessities and which were deliberate hardening choices. Firewall and transport changes were required to make backup traffic behave as intended. Resolver and home-directory corrections were required to make endpoint behavior match the documented identity model. By contrast, the `443`-only publication model and the decision to keep OPNsense GUI checks on the documented HTTP path were design-finalization choices that improved clarity and consistency without changing the service boundary itself.

**Table 3A. Delivery-phase change and finalization summary**

| Change Area | Earlier Observed Condition | Finalized Condition | Why the Change Was Needed |
|---|---|---|---|
| Veeam agent transport | Early backup troubleshooting showed hostname ambiguity and transport failures around the Veeam agent-management path | Veeam transport and control paths were normalized around the documented Site 2 backup host, required ports, and current administrative credential standard | Restored a reliable backup-control story and separated transport faults from repository or job-definition faults |
| Windows/Linux backup credentials | Earlier testing revealed stale or conflicting credential expectations in parts of the backup workflow | Current operational standard is `admin` for Linux systems and `.\Administrator` for Windows systems, with final GUI cleanup left as an administrative hygiene step rather than a blocker | Reduced credential drift and aligned the backup platform to the current environment inventory |
| Company 2 web publication | The web layer still needed to be tightened to the final hostname-first model | `C2WebServer` now serves on `443` only, returns `200` for the documented FQDN, and rejects the raw-IP path with `404` | Enforced the intended application-publishing model and removed ambiguity between host-header access and incidental address access |
| Dual-record DNS publication | Company 2 web namespace visibility temporarily drifted from the intended two-site model | `c2-webserver.c2.local` again carries both expected A records for Site 1 and Site 2 reachability | Restored consistency between the DNS design, client behavior, and the documented cross-site hostname narrative |
| `C2LinuxClient` resolver scope | The client had drifted into a single-namespace resolver posture and no longer represented the intended multi-namespace endpoint experience | The client now carries `c1.local` and `c2.local` in active search scope and resolves both web identities without raw-IP pinning | Made client behavior match the documented namespace model and removed address-based shortcuts from the service story |
| Domain-user shell readiness on `C2LinuxClient` | Domain users could resolve but did not initially present a complete endpoint experience | Domain-user home directories and user-context behavior were normalized so that real client sessions align with identity evidence | Turned identity lookup into practical client usability and made endpoint validation more credible |
| Test-tool alignment to source-of-truth inventory | Temporary workbook and toolkit defaults briefly diverged from the authoritative host inventory | Final validation reverted to the real host inventory, documented GUI port choices, and only retained the SMB retry logic as a justified test hardening patch | Prevented tooling assumptions from becoming architecture claims and kept the validation set tied to the real environment |
...
This platform layout is useful because it demonstrates role separation, shows intentional infrastructure design, and makes the environment easier to understand for an operations handoff without relying on unnecessary platform-specific background detail.
...
The two Company 1 client roles also support different operational perspectives. `C1UbuntuClient` is valuable for showing that the documented hostname model works cleanly from the Company 1 perspective, especially for cross-site HTTPS consumption and shell-visible domain context. `C1WindowsClient`, by contrast, is the more natural endpoint for presenting the lived Public or Private share workflow in the exact form that a Company 1 user would experience it. Treating those two clients as complementary rather than interchangeable makes the Company 1 narrative stronger and more realistic.
...
Both clients were able to resolve and reach both internal web hostnames. That result matters because it shows the same named services can be consumed consistently from both company perspectives:
...
| SMB consumption path | Additional Company 1 validation perspective | `c2fs.c2.local` used successfully for browse, public-share write or delete, and private-share access |
| Operational significance | Shows Company 1 can consume both tenant web names | Shows Company 2 can consume both tenant web names |
...
### Delivery-Phase Backup and Credential Refinements

The backup layer also went through a more visible delivery-phase refinement cycle than most other parts of the environment, and that history is useful for operations staff because it explains why the final backup section is written as a layered design rather than as a single repository note.

Early troubleshooting indicated that backup instability could not be reduced to one root cause. Name resolution, port reachability, repository targeting, and agent-management transport all had to be considered together. That led to a safer progression: first confirm the Veeam services and ports, then confirm repository and copy-target definitions, then isolate whether a symptom came from transport, identity, or a stale GUI-side credential object. This was a sound sequence because it avoided forcing repository redesign when the real issue might instead be a communication or credential path.

That refinement cycle also explains the final credential model. The environment was normalized toward a simple standard of `admin` for Linux systems and `.\Administrator` for Windows systems, while allowing the last GUI-side credential cleanup to happen without changing the broader design. In other words, the final backup posture depends on a stable control path and stable credential intent, not on preserving older credential objects indefinitely inside the console.

The result is a more defensible backup story. `S2Veeam` is not documented merely as a host that happens to exist. It is documented as the point where repository placement, job families, file-share protection, and offsite copy were all re-read and aligned to the same operational standard before final handover.
...
Requirement traceability is what separates a persuasive technical report from a simple build summary. In Site 2, each major requirement needs to be tied to a concrete implementation choice and to live or documentary evidence that confirms its delivered operating state.
...
|---|---|
| Confirm access to `Jump64` and `MSPUbuntuJump` | Without the bastion path, many healthy services become slower to assess |
