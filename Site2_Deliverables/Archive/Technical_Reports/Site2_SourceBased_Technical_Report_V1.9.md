...
|---|---|
| Document Type | Formal technical handover and operations report |
...
| Document Classification | Internal technical handover and operational review record |

**Engineering Contributors:** Bailey Kulla, Elyazid Sidelkheir, Ru Wang, Justin Rosseleve, Yiqin Huang, Omer Deniz

**Document Purpose:** This report explains the Site 2 architecture, operational logic, current operating state, and handover-ready support context using design inputs, environment evidence, official vendor documentation, and observed operating results collected for the Site 2 deployment.
...
**Figure 13.** `S2-Ubuntu-Client` Company 1 client dual-web evidence  
...
- current operating observations recorded on March 23 and March 24, 2026
...
- Company 1 core services through `C1DC1`, `C1DC2`, `C1FS`, `C1WindowsClient`, `S2-Ubuntu-Client`, `C1WebServer`, and `C1SAN`
...
The purpose of this document is to describe Site 2 in a form that supports operational handover, ongoing support, and formal service review. The report is intentionally written for a reader who may need to understand the environment quickly, assess its behavior, and support it afterward without relying on undocumented tribal knowledge.

This version is grounded in the available design inputs, the current Site 2 evidence set, official vendor documentation for the deployed technologies, and current operating observations recorded on March 23 and March 24, 2026. It is also written to reflect three senior perspectives at the same time: project delivery, network engineering, and systems administration.
...
| Current operating observations | March 23 and March 24, 2026 observations from the approved remote path, jump hosts, MSP bastion, and direct read-only SSH | Describes current operating state without configuration changes |
...
The observation approach followed a management-first bastion model. `MSPUbuntuJump` was treated as the primary bastion for low-risk CLI review into Company 1 and Company 2 paths. The Windows jump and local workstation were used only where they added practical reachability context. Direct read-only SSH was used only against systems where it could confirm state without configuration changes.
...
| Direct read-only SSH to Linux systems | To confirm service state without configuration changes |
...
| Final GUI screenshot placeholders | To reserve correct locations for Veeam, OPNsense, and interactive evidence |
...
| `S2-Ubuntu-Client` | Company 1 Linux client service role | Resolved both required web hostnames and showed Company 1 domain context | Client-side service access was visible from the Company 1 domain context |
...
**Figure 5. Company 1 services from the Site 2 management path.**  
Description: Insert a screenshot showing Company 1 service state from the Site 2 management path, such as cross-site port checks, Company 1 DNS records on Company 2 identity services, or the Company 1 web hostname responding from Site 2.  
...
| Domain-user state | `administrator@c1.local` active session | `employee1@c2.local` and `employee2@c2.local` resolved by `getent passwd` |
...
Current operating observations corroborated the management and control plane by showing that `S2Veeam` was reachable from the approved admin path on:
...
| Offsite copy to Site 1 | Environment design note plus OPNsense route and rule evidence | Offsite backup design is part of the deployed architecture |
...
|---|---|---|---|---|
| Secure remote access | `Jump64`, `MSPUbuntuJump`, approved remote operator access, and OPNsense WAN NAT only for jump systems | Bastion reachability checks plus gateway NAT review | Proven | Edge exposure remains limited to jump services only |
| Segmented multi-network design | OPNsense interfaces for `MSP`, `C1LAN`, `C1DMZ`, `C2LAN`, `C2DMZ`, plus isolated SAN bridges | Configuration review plus current path checks | Proven | Support with one OPNsense GUI screenshot |
| Site-to-site connectivity | `SITE1_OVPN`, `C1_REMOTE`, `C2_REMOTE`, Veeam route, and cross-site rules | Route or rule review plus Company 1 reachability from Site 2 | Proven | Use rule screenshots if a reviewer asks how traffic is controlled |
...
| Company 1 services | `C1DC1`, `C1DC2`, `C1FS`, `C1WebServer`, `C1WindowsClient`, `S2-Ubuntu-Client`, `C1SAN`, and shared namespace visibility | Inventory evidence, administrative-path checks, client access, DNS visibility, and SAN evidence | Proven | Document the current service set and operating state |
...
| Client-side service consumption | `S2-Ubuntu-Client` and `C2LinuxClient` resolving and reaching both required hostnames | Direct client-side web and identity checks | Proven | Confirms the current user-side service experience from both company contexts |
| Backup, file-share backup, and offsite copy | `S2Veeam`, Site 2 repository disk, file-share backup, and Site 1 offsite-copy design | Live port checks, route or rule review, and available backup design detail | Strongly supported | Needs one Veeam GUI screenshot for full closure |
| Linux-client SSH accessibility requirement | Linux client reachable through the approved management path | Internal SSH/admin path visible from Site 2 | Partially evidenced | If a reviewer requests explicit external-edge evidence, provide a current live test or screenshot from that access path |
...
| Company 1 services | `C1DC1`, `C1DC2`, `C1FS`, `C1WebServer`, `C1WindowsClient`, `S2-Ubuntu-Client`, `C1SAN`, and Site 1 remote aliases | Administrative-path reachability, client behavior, and environment inventory evidence | Company 1 service paths are unavailable from the expected operating flow | `nc` checks, hostname curl, inventory review, and client-side checks |
...
| Company 1 end users | `c1.local` user accounts | `C1WindowsClient`, `S2-Ubuntu-Client`, and Company 1 service paths | User authentication and normal service consumption | Reflects normal tenant use of Company 1 services |
...
| Interactive share-access evidence | `C2LinuxClient` identity state is strong, but an interactive screenshot of share access would further strengthen the package |
...
| OPNsense GUI capture | Configuration evidence is strong, but one GUI screenshot will communicate the design faster to a reviewer |
| Linux-client external-edge evidence | Internal and admin-path evidence are strong, but a specific external-edge SSH check was not separately repeated in this pass |

These are acceptable limitations for the current observation pass. They do not weaken the strong evidence already collected for segmentation, identity, DNS, storage, client behavior, internal web delivery, backup-path design, and requirement traceability.
...
|---|---|---|---|---|
| Project management and delivery | Does Site 2 satisfy the intended scope as one complete service environment | Yes; management, tenant service delivery, storage, and protection workflows are all represented and traceable | Design inputs, inventory evidence, current observations, and requirement traceability | Final screenshots to complete presentation polish |
| Network engineering | Is the network design segmented, supportable, and exposure-controlled | Yes; OPNsense shows bounded entry, inter-site routing, DNS reachability, and service-specific policy intent | OPNsense configuration evidence, NAT mappings, interface definitions, aliases, route logic, and current reachability checks | One GUI firewall screenshot for visual confirmation |
| Systems administration | Are identity, client access, file services, and backup paths operationally coherent | Yes; active services, client outcomes, storage sessions, share definitions, and backup control paths all align with the intended design | Samba AD and DHCP state, DNS records, client access, `C2FS` storage checks, and `S2Veeam` port reachability | Final Veeam console and interactive share screenshots |
...
| File services and share isolation | `C2FS` service checks, mount review, `testparm`, and sync-log review | SMB service state, mounted storage, public/private shares, and successful synchronization were all evidenced | High |
...
| External Linux SSH expectation | Administrative-path testing only in the current pass | Internal and managed access paths are supported, but public-edge evidence was not repeated in this revision | Medium |
