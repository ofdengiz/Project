...
| Document Version | 4.0 Final Revision |
...
**Figure 1.** C2IdM1 replication and DNS proof  
**Figure 2.** C2IdM2 replication and DNS proof  
**Figure 3.** C2LinuxClient hostname resolution proof  
**Figure 4.** C2LinuxClient SSSD and domain identity proof  
**Figure 5.** C2Web hostname-only hardening proof  
**Figure 6.** C2FS mounted storage and sync proof  
**Figure 7.** Veeam console evidence for Site 2 backup operations  
**Figure 8.** Site 2 offsite backup copy folders on the Site 1 SMB repository  
**Figure 9.** Public ClearRoots HTTPS landing page  
**Figure 10.** Kubernetes nodes and running ClearRoots web pods  
**Figure 11.** AWS EC2 instances for the ClearRoots public website  
**Figure 12.** Terraform remote state stored in Amazon S3  
**Figure 13.** Route53 public DNS record for `clearroots.omerdengiz.com`  
...
Operationally, the environment progressed through several troubleshooting stages before stabilization. Earlier testing identified issues with Windows jump routing behavior, DMZ web server reachability, C2LinuxClient SSSD identity resolution, the Site 2 to Site 1 sync path, and late-stage Veeam control-path problems. Those issues were later corrected. The final validated state shows healthy Company 2 identity services, restored web reachability, functioning Linux client identity lookup, successful file synchronization, healthy Kubernetes web deployment, and working Veeam local, file, and offsite backup flows after the March 23 remediation pass.
...
A final live validation pass was also run on March 23, 2026 from the operator workstation and through the approved Ubuntu jump path. That pass reconfirmed Tailscale reachability to both jump systems, healthy hostname-based HTTPS for `c1-webserver.c1.local` and `c2-webserver.c2.local`, successful `C2FS` synchronization on the March 23 `02:00` run, restored domain-user lookup for `employee1@c2.local` and `employee2@c2.local`, and reachable Veeam service ports `445`, `3389`, `9392`, `5985`, `10005`, and `10006` on `S2Veeam`. The only repeated limitation in the live collector was the Windows-jump WMI return channel, which is treated in this document as a tooling-path issue rather than as evidence of a Site 2 service outage.
...
| Ubuntu jump to C1 infrastructure | RDP/SMB reachable to C1DC1, C1DC2, C1DFS | Reliable cross-site validation path |
...
### 3.4.2 Public Cloud and Uplink Baseline
...
**Figure 1. `C2IdM1` final replication and DNS query proof from the approved admin path.**  
Description: Capture `samba-tool drs showrepl` and the local `c2-webserver.c2.local` DNS query on `C2IdM1`, showing `0 consecutive failure(s)` and the expected dual A records.  

| Figure 1 Placeholder |
|---|
| Insert the `C2IdM1` replication and DNS screenshot here before final submission. |
|  |
|  |
|  |

**Figure 2. `C2IdM2` final replication and DNS query proof from the approved admin path.**  
Description: Capture `samba-tool drs showrepl` and the local `c2-webserver.c2.local` DNS query on `C2IdM2`, showing `0 consecutive failure(s)` and the expected dual A records.  

| Figure 2 Placeholder |
|---|
| Insert the `C2IdM2` replication and DNS screenshot here before final submission. |
|  |
|  |
|  |
...
**Figure 3. `C2LinuxClient` hostname resolution proof for `c1-webserver.c1.local` and `c2-webserver.c2.local`.**  
Description: Show `nslookup` output on `C2LinuxClient` for both tenant web names with the expected dual-record answers.  

| Figure 3 Placeholder |
|---|
| Insert the `C2LinuxClient` DNS-resolution screenshot here before final submission. |
|  |
|  |
|  |

**Figure 4. `C2LinuxClient` SSSD, realm, and domain-user lookup proof.**  
Description: Show `realm list` and successful `getent passwd` results for Company 2 domain users, proving restored SSSD-backed identity lookup.  

| Figure 4 Placeholder |
|---|
| Insert the `C2LinuxClient` SSSD and domain-identity screenshot here before final submission. |
|  |
|  |
|  |

**Figure 5. `C2WebServer` Docker status and hostname-only hardening proof showing raw IP `404` and hostname `200`.**  
Description: Show that hostname-based HTTPS succeeds for `c2-webserver.c2.local` while raw IP access is intentionally hardened to return `404`.  

| Figure 5 Placeholder |
|---|
| Insert the `C2WebServer` hostname-only hardening screenshot here before final submission. |
|  |
|  |
|  |
...
**Figure 6. `C2FS` mounted storage, published directories, and successful sync-log proof.**  
Description: Show the mounted `/mnt/c2_public` volume and the latest sync log ending with `Sync completed successfully` on the March 23 run.  

| Figure 6 Placeholder |
|---|
| Insert the `C2FS` storage and sync screenshot here before final submission. |
|  |
|  |
|  |
...
Backup and recovery became one of the strongest Site 2 outcomes by the end of the project. After an extended troubleshooting phase, the team rebuilt the Site 2 Veeam environment, validated service listeners and repository paths, corrected the remaining control-path issues, and restored successful agent-based, file-based, and offsite backup behavior.

The final Site 2 Veeam implementation includes three layers:

- local agent-based backups
- file-share backups
- offsite backup copy to Site 1 through SMB

Several operational findings were important during the final remediation pass:

- time synchronization was an early hidden dependency and had to be corrected before later tests became trustworthy
- Windows Firewall on `S2Veeam` needed explicit inbound allowance for agent-management ports `10005` and `10006`
- older classic agent jobs conflicted with newly created Windows and Linux agent policies, producing `already processed by backup job` errors until the protection ownership was cleaned up
- OPNsense alias and rule coverage had to be aligned so that `S2_VEEAM` and the required Veeam control ports were reachable from the intended protected subnets

These points matter because the failing jobs did not behave like a damaged repository. They behaved like a control-path problem between protected hosts, the backup server, and the routed rule set. Once those dependencies were corrected, the backup platform returned to the expected working state.

### 3.8.1 Agent-based system backup

Site 2 Veeam was configured to protect a mixed set of Windows and Linux systems through agent-based protection. During the final correction phase, that design was refactored toward operating-system-specific groupings so that Windows and Linux systems would not compete for the same policy or classic backup job path. The final operating model keeps Windows and Linux protection logically separated and avoids overlapping ownership of the same endpoints.
...
| 172.30.65.4 | C1DFS | Windows agent backup |
...
The late remediation pass showed that keeping the same hosts in both old classic jobs and new agent policies caused avoidable conflicts. For the final handover, the correct operating model is:

- one Windows agent-based path per intended Windows grouping
- one Linux agent-based path per intended Linux grouping
- no overlap between classic jobs and policies for the same endpoint set
...
This design is appropriate because file-share retention and restore workflows are different from full system recovery workflows. It also made the later copy jobs easier to interpret in the offsite repository, where separate copy folders were created automatically by Veeam. During the March 23 live troubleshooting pass, these file jobs briefly exposed backup-server name/path dependencies, but those issues were later corrected as part of the final remediation.
...
This confirms that the intended Site 2 design no longer protects data only locally. It now includes an additional recovery location in Site 1, which materially improves resilience against local repository loss or single-site failure. The offsite repository structure existed throughout the implementation phase, and the final remediation restored the end-to-end backup workflow required for signoff.
...
|---|---|---|
| Windows agent-based job / policy set | Windows agent backup | Agent-based protection for Site 2 Windows hosts, grouped to avoid policy overlap |
| Linux agent-based job / policy set | Linux agent backup | Agent-based protection for Site 2 Linux hosts, grouped to avoid policy overlap |
| C1_FileShare | File backup | File-level protection for selected share content |
| C2_FileShare | File backup | File-level protection for selected share content |
| Site 2 offsite copy chain | Backup copy | Offsite copy to Site 1 SMB repository |
...
| Late agent remediation | Agent jobs timed out to `172.30.65.180:10005/10006` | Suggested Windows Firewall or routed-port problem | Listener checks on `S2Veeam`, firewall updates, and network-path correction restored connectivity |
| Policy conflict phase | New policies reported hosts already processed by other backup jobs | Suggested duplicate protection ownership | Old classic jobs and new policies were separated so each endpoint had one protection owner |
| OPNsense scope review | Site 2 protected hosts could not initially reach `S2Veeam` management ports | Suggested network-rule mismatch rather than Veeam service failure | Alias and rule-scope corrections were applied so the intended protected subnets could reach the Veeam control path |

This chronology is worth preserving because it captures one of the strongest technical lessons in the whole project: backup platforms can fail in ways that look like storage or application faults when the real issue is a control-path dependency between protected hosts, firewalls, policy ownership, and the backup server itself. Including that lesson makes the document much more useful as an operational handover artifact.

**Figure 7. Representative Veeam console evidence from Site 2 backup validation and troubleshooting.**  
Description: Show the resolved Veeam console state with successful or completed sessions for the final Site 2 backup design.  

| Figure 7 Placeholder |
|---|
| Insert the final Veeam console screenshot here before final submission. |
|  |
|  |
|  |

**Figure 8. Windows Explorer view of `R:\Repo_Site2_Offsite\Site2OffsiteFromSite2` showing backup-copy folders.**  
Description: Show the Site 1 offsite target folder structure proving that Site 2 backup-copy data reached the SMB repository.  

| Figure 8 Placeholder |
|---|
| Insert the offsite backup-copy folder screenshot here before final submission. |
|  |
|  |
|  |
...
**Figure 9. Public browser view of the ClearRoots Foundation landing page over HTTPS.**  
Description: Show the live public browser view of `https://clearroots.omerdengiz.com` with the address bar visible.  

| Figure 9 Placeholder |
|---|
| Insert the public ClearRoots HTTPS browser screenshot here before final submission. |
|  |
|  |
|  |

**Figure 10. `kubectl get nodes` and `kubectl get pods` showing both nodes Ready and the ClearRoots pods Running.**  
Description: Show terminal proof that both Kubernetes nodes are ready and the ClearRoots pods are running.  

| Figure 10 Placeholder |
|---|
| Insert the Kubernetes nodes and pods screenshot here before final submission. |
|  |
|  |
|  |

**Figure 11. AWS EC2 console showing `clearroots-kube-master` and `clearroots-kube-worker` in the Running state.**  
Description: Show the AWS EC2 inventory page with both ClearRoots instances in the running state.  

| Figure 11 Placeholder |
|---|
| Insert the AWS EC2 instances screenshot here before final submission. |
|  |
|  |
|  |

**Figure 12. Amazon S3 console showing the Terraform state object under `clearroots/k8s/terraform.tfstate`.**  
Description: Show the S3 bucket and Terraform state object used for the Site 2 public-cloud deployment.  

| Figure 12 Placeholder |
|---|
| Insert the S3 Terraform state screenshot here before final submission. |
|  |
|  |
|  |

**Figure 13. Route53 public hosted zone showing the A record for `clearroots.omerdengiz.com`.**  
Description: Show the final Route53 A record that publishes `clearroots.omerdengiz.com` to the worker Elastic IP.  

| Figure 13 Placeholder |
|---|
| Insert the Route53 hosted-zone screenshot here before final submission. |
|  |
|  |
|  |
...
Together, these demonstrate practical understanding of different certificate deployment models for internal and public services. Because this report is a Site 2 handover, the Lumora trust workflow is noted as a related cross-site practice rather than as a primary Site 2 figure-backed evidence set.
...
| Veeam local jobs | Veeam console, listener tests, and final remediation evidence | Healthy after remediation | Agent and file backup paths were corrected and validated after the March 23 fixes |
| Veeam offsite copy | SMB target folder and copy history | Healthy | Site 1 offsite folders and repository mapping remain valid and support the final backup story |
...
| 5 | Veeam and miscellaneous support evidence | Healthy after remediation | `S2Veeam` listener, firewall, policy, and path issues were corrected; final console screenshots should show the resolved state rather than the intermediate failures |
...
- verify that Windows and Linux agent-based protection paths do not overlap on the same endpoints
- verify success of the active Windows agent job or policy set
- verify success of the active Linux agent job or policy set
- verify success of `C1_FileShare`
- verify success of `C2_FileShare`
- verify success of copy jobs including the Site 2 offsite chain
- confirm offsite folders continue to populate under the Site 1 SMB repository
- confirm `172.30.65.180:10005` and `172.30.65.180:10006` remain reachable from protected subnets
...
| Backup operator | Local and offsite protection | Veeam job state, listener tests, offsite SMB repository folders, and final console proof | Veeam console, Site 2 firewall GUI, and Site 1 offsite share |
...
- the backup platform is implemented and operationally validated, but routine restore testing should still remain part of ongoing best practice
...
- Veeam now provides working local backup, file backup, and offsite backup copy workflows after the final remediation pass
...
|---|---|---|
| Windows agent-based protection set | Windows agent backup | Success |
| Linux agent-based protection set | Linux agent backup | Success |
| C1_FileShare | File backup | Success |
| C2_FileShare | File backup | Success |
| Historical backup-copy objects | Backup copy | Success |
| Site 2 offsite copy path | Backup copy | Success |
...
The March 23 combined validation set, including the refreshed live operator run and the approved Ubuntu-jump checks, confirmed that the core Site 2 service blocks were healthy. The remaining gaps are documentation-evidence gaps rather than unresolved service failures. To keep the final package disciplined, the remaining screenshots should be collected in the same order that the document introduces its claims, with Veeam left to the end.

The highest-value remaining non-Veeam captures are:

- OPNsense rule and route evidence that actually shows the approved inter-site path, especially the `OpenVPN`, `C1LAN`, and `C2LAN` rule views plus an actual route-status or route-table screen rather than a generic firewall image
- `C2LinuxClient` interactive share and private-isolation proof, which would strengthen Section `3.7`
- external browser and AWS console proof for the ClearRoots public site, which would strengthen Section `3.9.1`

The final screenshot pass should leave Veeam until last. Once the non-Veeam evidence is locked, the remaining Veeam captures should show only the resolved final state:

- repository and job-set screens that match the final agent-based design
- final success or completed-session history
- final listener / firewall / connectivity proof only if those images are still needed to support the written remediation summary

This ordering keeps the final Site 2 package aligned with the evidence density and discipline of the Site 1 final document while avoiding the earlier mistake of using loosely related screenshots in place of direct proof.
...
- Veeam console evidence demonstrates a fully implemented and working backup design after the final remediation pass.
- offsite repository folder evidence demonstrates that backup-copy traffic reached Site 1 as intended.
