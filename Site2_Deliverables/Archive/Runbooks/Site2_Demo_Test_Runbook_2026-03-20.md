...
4. Web and HTTPS services
5. File replication and storage
6. Backup, offsite copy, and public AWS site
...
- identity, storage, replication, backup, and public cloud publishing were all implemented
...
## Role 4: HTTPS and application services

Responsibilities:
- show Company 1 internal web
- show Company 2 internal web
- show public AWS-hosted ClearRoots website

### Internal web tests from Ubuntu jump

```bash
curl -k -I http://172.30.65.162
curl -k -I https://172.30.65.162
curl -I http://172.30.65.170
```

Expected outcome:
- `172.30.65.162` returns HTTP 200 and HTTPS 200
- `172.30.65.170` returns HTTP 200
...
### Public AWS website proof

Open in browser:
- `https://clearroots.omerdengiz.com`

Optional command proof:
```bash
curl -I https://clearroots.omerdengiz.com
```

### Talking point

“Site 2 includes both internal tenant-facing web delivery and a public AWS HTTPS publication workflow for the ClearRoots Foundation site.”
...
## Role 6: Backup, offsite copy, and cloud operations

Responsibilities:
- show Site 2 Veeam success
- show offsite copy folder on Site 1 target
- show AWS EC2 / Route53 / S3 state screenshots if needed
...
Show:
- EC2 instances for `clearroots-kube-master` and `clearroots-kube-worker`
- Route53 A record for `clearroots.omerdengiz.com`
- S3 Terraform state object
- browser view of public HTTPS site
- `kubectl get nodes`
- `kubectl get pods`
...
Role 4:
- show Company 1 and Company 2 internal web
- show public ClearRoots HTTPS site
...
- show AWS screenshots or live console if needed
...
- prioritize these six proofs:
  ...
  4. file sync log
  5. Veeam success
  6. public AWS HTTPS site

## 5. Must-have screenshots for the live demo deck

1. Service block matrix
2. AWS EC2 instance list
3. public ClearRoots website
4. `kubectl get nodes` and `kubectl get pods`
5. S3 state bucket object
6. Route53 record
7. Veeam success history
8. offsite copy target folder
9. OPNsense interfaces / OpenVPN / NAT
10. C2FS sync evidence
