...
## Priority 1: Management and Resolver Proof

1. `Jump64` final DNS settings
   Supports: Section `3.2` and Section `3.10.2`
   Claim proved: the Windows jump host became usable only after the final DNS/interface correction.
   Exact screen needed: Windows adapter `Advanced TCP/IP Settings > DNS` showing the final DNS server order.

2. `mspubuntujump` netplan and resolver state
   Supports: Section `3.2`, Section `3.6`, and Section `3.10.2`
   Claim proved: the Ubuntu jump was the most reliable validation vantage point because its resolver path was aligned for both internal domains.
   Exact screens needed:
   - `/etc/netplan/01-netcfg.yaml`
   - `resolvectl status`

## Priority 2: Route and Firewall Evidence

3. Site 2 OPNsense routed-path proof
...
## Priority 3: File Services and Client Isolation

5. `C2LinuxClient` interactive private-share isolation
...
## Priority 4: Public Cloud Proof

6. External browser proof for `https://clearroots.omerdengiz.com`
...
8. `S2Veeam` final job-state proof
...
9. `S2Veeam` final repository and listener proof
   Supports: Section `3.8`
...
10. Final host-to-VBR connectivity proof
    Supports: Section `3.8`
