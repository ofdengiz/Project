...
The earlier March 22 list is now largely closed. The items below are the remaining screenshots that would add the most value to the revised Site 2 final package after the March 23 Veeam and OPNsense troubleshooting pass.
...
3. `S2Veeam` final success proof
   Capture:
   - the job list showing the final Windows/Linux/file job set
   ...
   - one final success-history or completed-session screenshot
...
5. `C2LinuxClient` live connectivity proof for Veeam
   ...
   This is useful now as a final after-fix connectivity proof.
