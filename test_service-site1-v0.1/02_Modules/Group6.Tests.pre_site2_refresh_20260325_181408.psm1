...
"@).Trim()
    ...
    foreach ($owner in $specs) {
        ...
        foreach ($viewer in $specs | Where-Object { $_.Token -ne $ownerToken }) {
            ...
            $commands += (@"
smbclient //c2fs.c2.local/C2_Public -W C2 -U $($viewer.MountUsername)%Cisco123! -c "ls $remoteFile" > '$resultFile' 2>&1 || true; cat '$resultFile'; grep -Fq '$remoteFile' '$resultFile' && echo $visibleMarker || echo $hiddenMarker; rm -f '$resultFile'
"@).Trim()
...
"@).Trim()
    ...
    foreach ($owner in $specs) {
        ...
        foreach ($viewer in $specs | Where-Object { $_.Token -ne $ownerToken }) {
            ...
            $commands += (@"
smbclient //c2fs.c2.local/C2_Private -W C2 -U $($viewer.MountUsername)%Cisco123! -c "ls $remoteFile" > '$resultFile' 2>&1 || true; cat '$resultFile'; grep -Fq '$remoteFile' '$resultFile' && echo $visibleMarker || echo $hiddenMarker; rm -f '$resultFile'
"@).Trim()
