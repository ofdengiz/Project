...
function Invoke-Group6SB2Hotseat1AdditionalTests {
    ...
    Invoke-ToolkitWinrmTest -Name 'C1 secondary DNS resolution summary' -ComputerName $c1dc2.Address -Device $c1dc2.DisplayName -DocSection 'Service Block 2 / DNS' -CommandLines @(
        ...
        $forwarders = @(Get-DnsServerForwarder | Select-Object -ExpandProperty IPAddressToString)
    ...
    Invoke-ToolkitSshTest -Name 'C1-Client2 hostname-only company-web validation' -Host $c1Client2.Address -User $c1Client2.LinuxUser -Device $c1Client2.DisplayName -DocSection 'Service Block 2 / HTTPS' -Commands @(
        'code=$(curl -sk -o /dev/null -w "%{http_code}" https://c1-webserver.c1.local/ || true); if [ "$code" = "200" ]; then echo C1CLIENT_HTTPS_FQDN_OK; else echo C1CLIENT_HTTPS_FQDN_FAIL:$code; fi',
        'code=$(curl -sk -o /dev/null -w "%{http_code}" https://172.30.64.162/ || true); if [ "$code" = "403" ] || [ "$code" = "404" ] || [ "$code" = "000" ]; then echo C1CLIENT_HTTPS_IP_HARDENED; else echo C1CLIENT_HTTPS_IP_ALLOWED:$code; fi',
        'code=$(curl -s -o /dev/null -w "%{http_code}" http://172.30.64.162/ || true); if [ "$code" = "403" ] || [ "$code" = "404" ] || [ "$code" = "000" ]; then echo C1CLIENT_HTTP_IP_HARDENED; else echo C1CLIENT_HTTP_IP_ALLOWED:$code; fi',
        'code=$(curl -s -o /dev/null -w "%{http_code}" http://c1-webserver.c1.local/ || true); if [ "$code" = "403" ] || [ "$code" = "404" ] || [ "$code" = "000" ]; then echo C1CLIENT_HTTP_FQDN_HARDENED; else echo C1CLIENT_HTTP_FQDN_ALLOWED:$code; fi'
    ) -DisplayCommands @(
        'curl -sk -o /dev/null -w "%{http_code}" https://c1-webserver.c1.local/',
        'curl -sk -o /dev/null -w "%{http_code}" https://172.30.64.162/',
        'curl -s -o /dev/null -w "%{http_code}" http://172.30.64.162/',
        'curl -s -o /dev/null -w "%{http_code}" http://c1-webserver.c1.local/'
    ) -SuccessPatterns @('C1CLIENT_HTTPS_FQDN_OK', 'C1CLIENT_HTTPS_IP_HARDENED', 'C1CLIENT_HTTP_IP_HARDENED', 'C1CLIENT_HTTP_FQDN_HARDENED') -CheckDescription 'From C1-Client2, confirm the Company 1 website is consumed only through https://c1-webserver.c1.local while HTTP and direct-IP access remain blocked.'
}
...
function Invoke-Group6SB2Hotseat1Site2Tests {
    ...
    Invoke-ToolkitSshTest -Name 'Site 2 Company 1 web hostname-only access summary' -Host $mspJump.Address -User $mspJump.LinuxUser -Device 'MSPUbuntuJump -> Site 2 C1 web' -DocSection 'Service Block 2 / Hotseat 1 / Site 2 Company 1 web' -Commands @(
        ...
        'code=$(curl -k -s -o /dev/null -w "%{http_code}" https://172.30.65.162/ || true); if [ "$code" = "403" ] || [ "$code" = "404" ] || [ "$code" = "000" ]; then echo S2_C1WEB_HTTPS_IP_HARDENED; else echo S2_C1WEB_HTTPS_IP_ALLOWED:$code; fi',
        'code=$(curl -s -o /dev/null -w "%{http_code}" http://172.30.65.162/ || true); if [ "$code" = "403" ] || [ "$code" = "404" ] || [ "$code" = "000" ]; then echo S2_C1WEB_HTTP_IP_HARDENED; else echo S2_C1WEB_HTTP_IP_ALLOWED:$code; fi',
        'code=$(curl -s -o /dev/null -w "%{http_code}" http://c1-webserver.c1.local/ || true); if [ "$code" = "403" ] || [ "$code" = "404" ] || [ "$code" = "000" ]; then echo S2_C1WEB_HTTP_FQDN_HARDENED; else echo S2_C1WEB_HTTP_FQDN_ALLOWED:$code; fi'
    ) -DisplayCommands @(
        'curl -k -s -o /dev/null -w "%{http_code}" https://c1-webserver.c1.local/',
        'curl -k -s -o /dev/null -w "%{http_code}" https://172.30.65.162/',
        'curl -s -o /dev/null -w "%{http_code}" http://172.30.65.162/',
        'curl -s -o /dev/null -w "%{http_code}" http://c1-webserver.c1.local/'
    ) -SuccessPatterns @('S2_C1WEB_FQDN_OK', 'S2_C1WEB_HTTPS_IP_HARDENED', 'S2_C1WEB_HTTP_IP_HARDENED', 'S2_C1WEB_HTTP_FQDN_HARDENED') -CheckDescription 'From Site 2 MSPUbuntuJump, confirm Company 1 web access succeeds only through the HTTPS hostname while direct-IP and HTTP access remain hardened.'
}
...
function Invoke-Group6SB2Hotseat2AdditionalTests {
    ...
    ) -SuccessPatterns @('SECONDARY_C2DC1_OK', 'SECONDARY_C2DC2_OK', 'SECONDARY_C1WEB_OK', 'SECONDARY_C2WEB_OK', 'SECONDARY_EXTERNAL_LOOKUP_OK') -CheckDescription 'On C2-DC2, confirm the secondary Company 2 DNS node can still resolve controller records, both tenant web names, and recursive external lookups for failover readiness.'
    ...
    Invoke-ToolkitSshTest -Name 'Site 2 C2LinuxClient company-web hostname-only enforcement' -Host $c2LinuxClient.Address -User $c2LinuxClient.LinuxUser -Device $c2LinuxClient.DisplayName -DocSection 'Service Block 2 / Hotseat 2 / Site 2 Company 2 client' -Commands @(
        'code=$(curl -k -s -o /dev/null -w "%{http_code}" https://c2-webserver.c2.local/ || true); if [ "$code" = "200" ]; then echo S2_C2CLIENT_C2WEB_HTTPS_FQDN_OK; else echo S2_C2CLIENT_C2WEB_HTTPS_FQDN_FAIL:$code; fi',
        'code=$(curl -k -s -o /dev/null -w "%{http_code}" https://172.30.65.170/ || true); if [ "$code" = "403" ] || [ "$code" = "404" ] || [ "$code" = "000" ]; then echo S2_C2CLIENT_C2WEB_HTTPS_IP_HARDENED; else echo S2_C2CLIENT_C2WEB_HTTPS_IP_ALLOWED:$code; fi',
        'code=$(curl -s -o /dev/null -w "%{http_code}" http://172.30.65.170/ || true); if [ "$code" = "403" ] || [ "$code" = "404" ] || [ "$code" = "000" ]; then echo S2_C2CLIENT_C2WEB_HTTP_IP_HARDENED; else echo S2_C2CLIENT_C2WEB_HTTP_IP_ALLOWED:$code; fi',
        'code=$(curl -s -o /dev/null -w "%{http_code}" http://c2-webserver.c2.local/ || true); if [ "$code" = "403" ] || [ "$code" = "404" ] || [ "$code" = "000" ]; then echo S2_C2CLIENT_C2WEB_HTTP_FQDN_HARDENED; else echo S2_C2CLIENT_C2WEB_HTTP_FQDN_ALLOWED:$code; fi'
    ) -DisplayCommands @(
...
function Invoke-Group6ServiceBlock4 { [CmdletBinding()] param() Invoke-SB4-Hotseat1Tests; Invoke-SB4-Hotseat1AdditionalTests; Invoke-SB4-Hotseat2Tests; Invoke-Group6SB4Hotseat1Site2Tests; Invoke-Group6SB4Hotseat2Site2Tests }
