function Get-ServiceBlockCoverage {
    ...
    @(
        ...
        [pscustomobject]@{ Block = '4'; Hotseat = '1'; Area = 'Replicated File Server'; Coverage = 'Company 1 DFS namespace, DFSR service and replicated-folder evidence, SMB shares, C1-Client1 mapped-drive context evidence, and C1-Client2 per-user Linux share mounts plus temporary Public/Private file create, cross-user visibility, and isolation proof'; Mode = 'Automated + server-side' }
        ...
        [pscustomobject]@{ Block = '4'; Hotseat = '2'; Area = 'Replicated File Server'; Coverage = 'Company 2 Gluster, Samba public/private share definitions, Site 1 client share mounts plus temporary Public/Private file create and isolation proof, and Site 2 hostname-based SMB access with active write verification'; Mode = 'Automated' }
...
function New-LinuxMountedPublicWriteWorkflowPlan {
    param(
        [Parameter(Mandatory)]
        [string]$SharePrefix,
    ...
    $specs = Resolve-LinuxUserSpecs -UserSpecs $UserSpecs
    ...
    foreach ($spec in $specs) {
        $token = $spec.Token
        $sharePath = "$($spec.HomePath)/${SharePrefix}_Public"
        $fileName = "toolkit_{0}_{1}_{2}_public.txt" -f $SharePrefix.ToLowerInvariant(), $spec.MountUsername, $stamp
        $filePath = "$sharePath/$fileName"
        $content = "creator=$($spec.MountUsername) scope=public stamp=$stamp"

        $commands += (@"
printf "%s\n" "Cisco123!" | sudo -S -p "" -u '$($spec.RunAsUser)' sh -c 'printf "%s\n" "$content" > "$filePath"' && echo ${token}_PUBLIC_CREATE_OK && echo ${token}_PUBLIC_FILE=$fileName || echo ${token}_PUBLIC_CREATE_FAIL
"@).Trim()
        $commands += (@"
printf "%s\n" "Cisco123!" | sudo -S -p "" -u '$($spec.RunAsUser)' grep -Fq '$content' '$filePath' && echo ${token}_PUBLIC_SELF_FILE_OK || echo ${token}_PUBLIC_SELF_FILE_FAIL
"@).Trim()

        $successPatterns.Add("${token}_PUBLIC_CREATE_OK")
        $successPatterns.Add("${token}_PUBLIC_SELF_FILE_OK")
    }
    ...
    foreach ($owner in $specs) {
        ...
        $fileName = "toolkit_{0}_{1}_{2}_public.txt" -f $SharePrefix.ToLowerInvariant(), $owner.MountUsername, $stamp
        $content = "creator=$($owner.MountUsername) scope=public stamp=$stamp"

        foreach ($viewer in $specs | Where-Object { $_.Token -ne $ownerToken }) {
            $viewerToken = $viewer.Token
            $viewerPath = "$($viewer.HomePath)/${SharePrefix}_Public/$fileName"
            $visibleMarker = "${ownerToken}_PUBLIC_VISIBLE_TO_${viewerToken}"
            $hiddenMarker = "${ownerToken}_PUBLIC_HIDDEN_FROM_${viewerToken}"

            $commands += (@"
printf "%s\n" "Cisco123!" | sudo -S -p "" -u '$($viewer.RunAsUser)' grep -Fq '$content' '$viewerPath' && echo $visibleMarker || echo $hiddenMarker
"@).Trim()
    ...
    foreach ($spec in $specs) {
        ...
        $commands += (@"
printf "%s\n" "Cisco123!" | sudo -S -p "" -u '$($spec.RunAsUser)' sh -c 'test -f "$filePath" && rm -f "$filePath"' && echo ${token}_PUBLIC_CLEANUP_OK || echo ${token}_PUBLIC_CLEANUP_FAIL
"@).Trim()
    ...
    return [pscustomobject]@{
        ...
        DisplayCommands = @(
            'Warm up the per-user share sessions'
            ("Create a temporary file in each user's {0}_Public path" -f $SharePrefix)
            ("Confirm the other users can read those temporary files through their own {0}_Public path" -f $SharePrefix)
            'Delete the temporary Public test files after verification'
        )
    }
}

function New-LinuxMountedPrivateWriteWorkflowPlan {
    param(
        [Parameter(Mandatory)]
        [string]$SharePrefix,
    ...
    foreach ($spec in $specs) {
        $token = $spec.Token
        $sharePath = "$($spec.HomePath)/${SharePrefix}_Private"
        $fileName = "toolkit_{0}_{1}_{2}_private.txt" -f $SharePrefix.ToLowerInvariant(), $spec.MountUsername, $stamp
        $filePath = "$sharePath/$fileName"
        $content = "creator=$($spec.MountUsername) scope=private stamp=$stamp"

        $commands += (@"
printf "%s\n" "Cisco123!" | sudo -S -p "" -u '$($spec.RunAsUser)' sh -c 'printf "%s\n" "$content" > "$filePath"' && echo ${token}_PRIVATE_CREATE_OK && echo ${token}_PRIVATE_FILE=$fileName || echo ${token}_PRIVATE_CREATE_FAIL
"@).Trim()
        $commands += (@"
printf "%s\n" "Cisco123!" | sudo -S -p "" -u '$($spec.RunAsUser)' grep -Fq '$content' '$filePath' && echo ${token}_PRIVATE_SELF_FILE_OK || echo ${token}_PRIVATE_SELF_FILE_FAIL
"@).Trim()

        $successPatterns.Add("${token}_PRIVATE_CREATE_OK")
        $successPatterns.Add("${token}_PRIVATE_SELF_FILE_OK")
    }
    ...
    foreach ($owner in $specs) {
        $ownerToken = $owner.Token
        $fileName = "toolkit_{0}_{1}_{2}_private.txt" -f $SharePrefix.ToLowerInvariant(), $owner.MountUsername, $stamp

        foreach ($viewer in $specs | Where-Object { $_.Token -ne $ownerToken }) {
            $viewerToken = $viewer.Token
            $viewerPath = "$($viewer.HomePath)/${SharePrefix}_Private/$fileName"
            $hiddenMarker = "${ownerToken}_PRIVATE_HIDDEN_FROM_${viewerToken}"
            $visibleMarker = "${ownerToken}_PRIVATE_VISIBLE_TO_${viewerToken}"

            $commands += (@"
printf "%s\n" "Cisco123!" | sudo -S -p "" -u '$($viewer.RunAsUser)' test ! -e '$viewerPath' && echo $hiddenMarker || echo $visibleMarker
"@).Trim()
    ...
    foreach ($spec in $specs) {
        ...
        $commands += (@"
printf "%s\n" "Cisco123!" | sudo -S -p "" -u '$($spec.RunAsUser)' sh -c 'test -f "$filePath" && rm -f "$filePath"' && echo ${token}_PRIVATE_CLEANUP_OK || echo ${token}_PRIVATE_CLEANUP_FAIL
"@).Trim()
    ...
    return [pscustomobject]@{
        ...
        DisplayCommands = @(
            'Warm up the per-user share sessions'
            ("Create a temporary file in each user's {0}_Private path" -f $SharePrefix)
            ("Confirm the other users do not see those temporary files through their own {0}_Private path" -f $SharePrefix)
...
function Invoke-SB2-Hotseat1Tests {
    ...
    Invoke-ToolkitSshTest -Name 'C1 web application HTTPS response' -Host (Get-ToolkitHost 'C1Client2').Address -User (Get-ToolkitHost 'C1Client2').LinuxUser -Device 'C1-Client2' -DocSection 'Service Block 2 / HTTPS' -SuccessPatterns @('HTTPS_FQDN_STATUS=200', 'HTTPS_FQDN_BODY_OK') -CheckDescription 'From C1-Client2, request the Company 1 web page over HTTPS and confirm the tenant DNS host name returns a valid response.' -Commands @(
        'curl -sk --connect-timeout 5 --max-time 10 -o /dev/null -w "HTTPS_FQDN_STATUS=%{http_code}\n" https://c1-webserver.c1.local/',
        'curl -sk --connect-timeout 5 --max-time 10 https://c1-webserver.c1.local/ >/dev/null 2>&1 && echo HTTPS_FQDN_BODY_OK || echo HTTPS_FQDN_BODY_FAIL'
    )
    Invoke-ToolkitSshTest -Name 'C1 web application non-FQDN rejection' -Host (Get-ToolkitHost 'C1Client2').Address -User (Get-ToolkitHost 'C1Client2').LinuxUser -Device 'C1-Client2' -DocSection 'Service Block 2 / HTTPS' -SuccessPatterns @('HTTPS_IP_REJECTED', 'HTTP_IP_REJECTED', 'HTTP_FQDN_REJECTED') -MissingEvidenceStatus 'FAIL' -CheckDescription 'From C1-Client2, confirm the Company 1 website only permits https://c1-webserver.c1.local and that direct-IP or HTTP-based requests do not return the tenant page with HTTP 200.' -Commands @(
        'code=$(curl -sk --connect-timeout 5 --max-time 10 -o /dev/null -w "%{http_code}" https://172.30.64.162/ || true); if [ "$code" != "200" ]; then echo HTTPS_IP_REJECTED; else echo HTTPS_IP_ALLOWED:$code; fi',
        'code=$(curl -s --connect-timeout 5 --max-time 10 -o /dev/null -w "%{http_code}" http://172.30.64.162/ || true); if [ "$code" != "200" ]; then echo HTTP_IP_REJECTED; else echo HTTP_IP_ALLOWED:$code; fi',
        'code=$(curl -s --connect-timeout 5 --max-time 10 -o /dev/null -w "%{http_code}" http://c1-webserver.c1.local/ || true); if [ "$code" != "200" ]; then echo HTTP_FQDN_REJECTED; else echo HTTP_FQDN_ALLOWED:$code; fi'
    )
}
...
function Invoke-SB2-Hotseat2Tests {
    ...
    Invoke-ToolkitSshTest -Name 'C2 web application non-FQDN rejection' -Host (Get-ToolkitHost 'C2Client1').Address -User (Get-ToolkitHost 'C2Client1').LinuxUser -Device 'C2-Client1' -DocSection 'Service Block 2 / HTTPS' -SuccessPatterns @('HTTPS_IP_REJECTED', 'HTTP_IP_REJECTED', 'HTTP_FQDN_REJECTED') -MissingEvidenceStatus 'FAIL' -CheckDescription 'From C2-Client1, confirm the Company 2 website only permits https://c2-webserver.c2.local and that direct-IP or HTTP-based requests do not return the tenant page with HTTP 200.' -Commands @(
        'code=$(curl -sk -o /dev/null -w "%{http_code}" https://172.30.64.170/ || true); if [ "$code" != "200" ]; then echo HTTPS_IP_REJECTED; else echo HTTPS_IP_ALLOWED:$code; fi',
        'code=$(curl -s -o /dev/null -w "%{http_code}" http://172.30.64.170/ || true); if [ "$code" != "200" ]; then echo HTTP_IP_REJECTED; else echo HTTP_IP_ALLOWED:$code; fi',
        'code=$(curl -s -o /dev/null -w "%{http_code}" http://c2-webserver.c2.local/ || true); if [ "$code" != "200" ]; then echo HTTP_FQDN_REJECTED; else echo HTTP_FQDN_ALLOWED:$code; fi'
    )
}
...
function Invoke-SB4-Hotseat1Tests {
    ...
    $c1PublicWritePlan = New-LinuxMountedPublicWriteWorkflowPlan -SharePrefix 'C1' -Platform 'C1' -UserSpecs (Get-C1Client2LinuxUserSpecs)
    Invoke-ToolkitSshTest -Name 'C1-Client2 public share create-and-visibility summary' -Host (Get-ToolkitHost 'C1Client2').Address -User (Get-ToolkitHost 'C1Client2').LinuxUser -Device 'C1-Client2' -DocSection 'Service Block 4 / Replicated File Server' -SuccessPatterns $c1PublicWritePlan.SuccessPatterns -DisplayCommands $c1PublicWritePlan.DisplayCommands -MissingEvidenceStatus 'FAIL' -CommandTimeoutSeconds 90 -CheckDescription 'On C1-Client2, create a temporary file in each user''s Company 1 Public mount, confirm the other users can see it through their own Public path, and then remove the temporary files.' -Commands $c1PublicWritePlan.Commands

    $c1PrivateWritePlan = New-LinuxMountedPrivateWriteWorkflowPlan -SharePrefix 'C1' -Platform 'C1' -UserSpecs (Resolve-LinuxUserSpecs -Users @('employee1', 'employee2') -UserSpecs (Get-C1Client2LinuxUserSpecs))
    Invoke-ToolkitSshTest -Name 'C1-Client2 private share create-and-isolation summary' -Host (Get-ToolkitHost 'C1Client2').Address -User (Get-ToolkitHost 'C1Client2').LinuxUser -Device 'C1-Client2' -DocSection 'Service Block 4 / Replicated File Server' -SuccessPatterns $c1PrivateWritePlan.SuccessPatterns -DisplayCommands $c1PrivateWritePlan.DisplayCommands -MissingEvidenceStatus 'FAIL' -CommandTimeoutSeconds 90 -CheckDescription 'On C1-Client2, create a temporary file in each domain user''s Company 1 Private mount, confirm the other domain user''s own Private path does not expose it, and then remove the temporary files.' -Commands $c1PrivateWritePlan.Commands
    ...
    Invoke-ToolkitWinrmTest -Name 'C1-Client1 mapped drive context summary' -ComputerName (Get-ToolkitHost 'C1Client1').Address -Device 'C1-Client1' -DocSection 'Service Block 4 / Replicated File Server' -SuccessPatterns @(
        'EMPLOYEE1_PROFILE_PRESENT',
        'EMPLOYEE2_PROFILE_PRESENT',
        'CLIENT1_NAMESPACE_NAME_RESOLUTION_OK',
        'CLIENT1_C1DC1_SMB_OK',
        'CLIENT1_C1DC2_SMB_OK',
        'CLIENT1_C1FS_SMB_OK',
        'CLIENT1_MAPPED_DRIVE_CONTEXT_PASS'
    ) -FailPatterns @(
        'CLIENT1_MAPPED_DRIVE_CONTEXT_FAIL'
    ) -MissingEvidenceStatus 'FAIL' -CheckDescription 'On C1-Client1, confirm the employee1 and employee2 user profiles exist locally, the client can resolve the Company 1 namespace, and SMB reachability to the documented DFS targets is present from the current non-interactive validation context.' -CommandLines @(
        'Get-CimInstance Win32_UserProfile'
        'Resolve-DnsName -Name c1.local'
        'Test-NetConnection -ComputerName 172.30.64.130 -Port 445'
        'Test-NetConnection -ComputerName 172.30.64.131 -Port 445'
        'Test-NetConnection -ComputerName 172.30.65.4 -Port 445'
    ) -ScriptBlock {
        $profiles = @(Get-CimInstance Win32_UserProfile -ErrorAction SilentlyContinue)
        $employee1Profile = $profiles | Where-Object { $_.LocalPath -eq 'C:\Users\employee1' }
        $employee2Profile = $profiles | Where-Object { $_.LocalPath -eq 'C:\Users\employee2' }
        $namespaceLookup = @(Resolve-DnsName -Name 'c1.local' -ErrorAction SilentlyContinue)
        $c1dc1Smb = Test-NetConnection -ComputerName '172.30.64.130' -Port 445 -InformationLevel Quiet -WarningAction SilentlyContinue
        $c1dc2Smb = Test-NetConnection -ComputerName '172.30.64.131' -Port 445 -InformationLevel Quiet -WarningAction SilentlyContinue
        $c1fsSmb = Test-NetConnection -ComputerName '172.30.65.4' -Port 445 -InformationLevel Quiet -WarningAction SilentlyContinue

        if ($employee1Profile) { 'EMPLOYEE1_PROFILE_PRESENT' } else { 'EMPLOYEE1_PROFILE_MISSING' }
        if ($employee2Profile) { 'EMPLOYEE2_PROFILE_PRESENT' } else { 'EMPLOYEE2_PROFILE_MISSING' }
        if ($namespaceLookup) { 'CLIENT1_NAMESPACE_NAME_RESOLUTION_OK' } else { 'CLIENT1_NAMESPACE_NAME_RESOLUTION_FAIL' }
        if ($c1dc1Smb) { 'CLIENT1_C1DC1_SMB_OK' } else { 'CLIENT1_C1DC1_SMB_FAIL' }
        if ($c1dc2Smb) { 'CLIENT1_C1DC2_SMB_OK' } else { 'CLIENT1_C1DC2_SMB_FAIL' }
        if ($c1fsSmb) { 'CLIENT1_C1FS_SMB_OK' } else { 'CLIENT1_C1FS_SMB_FAIL' }

        if ($employee1Profile -and $employee2Profile -and $namespaceLookup -and $c1dc1Smb -and $c1dc2Smb -and $c1fsSmb) {
            'CLIENT1_MAPPED_DRIVE_CONTEXT_PASS'
        }
        else {
            'CLIENT1_MAPPED_DRIVE_CONTEXT_FAIL'
        ...
        $profiles | Where-Object { $_.LocalPath -in @('C:\Users\employee1', 'C:\Users\employee2') } | Select-Object SID, LocalPath, Loaded | Format-Table -HideTableHeaders
        $namespaceLookup | Select-Object -First 5 Name, QueryType, IPAddress | Format-Table -HideTableHeaders
        [pscustomobject]@{ Target = '172.30.64.130:445'; Reachable = [bool]$c1dc1Smb }
        [pscustomobject]@{ Target = '172.30.64.131:445'; Reachable = [bool]$c1dc2Smb }
        [pscustomobject]@{ Target = '172.30.65.4:445'; Reachable = [bool]$c1fsSmb }
    }
}
...
function Invoke-SB4-Hotseat2Tests {
    ...
    $c2PublicWritePlan = New-LinuxMountedPublicWriteWorkflowPlan -SharePrefix 'C2' -Platform 'C2' -UserSpecs (Get-C2Client1LinuxUserSpecs)
    Invoke-ToolkitSshTest -Name 'C2-Client1 public share create-and-visibility summary' -Host (Get-ToolkitHost 'C2Client1').Address -User (Get-ToolkitHost 'C2Client1').LinuxUser -Device 'C2-Client1' -DocSection 'Service Block 4 / Replicated File Server' -SuccessPatterns $c2PublicWritePlan.SuccessPatterns -DisplayCommands $c2PublicWritePlan.DisplayCommands -MissingEvidenceStatus 'FAIL' -CommandTimeoutSeconds 90 -CheckDescription 'On C2-Client1, create a temporary file in each user''s Company 2 Public mount, confirm the other users can see it through their own Public path, and then remove the temporary files.' -Commands $c2PublicWritePlan.Commands

    $c2PrivateWritePlan = New-LinuxMountedPrivateWriteWorkflowPlan -SharePrefix 'C2' -Platform 'C2' -UserSpecs (Get-C2Client1LinuxUserSpecs)
    Invoke-ToolkitSshTest -Name 'C2-Client1 private share create-and-isolation summary' -Host (Get-ToolkitHost 'C2Client1').Address -User (Get-ToolkitHost 'C2Client1').LinuxUser -Device 'C2-Client1' -DocSection 'Service Block 4 / Replicated File Server' -SuccessPatterns $c2PrivateWritePlan.SuccessPatterns -DisplayCommands $c2PrivateWritePlan.DisplayCommands -MissingEvidenceStatus 'FAIL' -CommandTimeoutSeconds 90 -CheckDescription 'On C2-Client1, create a temporary file in each user''s Company 2 Private mount, confirm each other user''s own Private path does not expose it, and then remove the temporary files.' -Commands $c2PrivateWritePlan.Commands
}
