function Get-ServiceBlockCoverage {
    ...
    @(
        ...
        [pscustomobject]@{ Block = '1'; Hotseat = '1'; Area = 'Account Administration'; Coverage = 'Company 1 AD users, named workflow identities, groups, managed endpoint alignment, C1-Client2 user contexts, and C1-Client1 domain-login evidence'; Mode = 'Automated' }
        ...
        [pscustomobject]@{ Block = '1'; Hotseat = '2'; Area = 'Account Administration'; Coverage = 'Company 2 Samba users, named workflow identities, groups, and C2-Client1 user contexts'; Mode = 'Automated' }
        ...
        [pscustomobject]@{ Block = '2'; Hotseat = '1'; Area = 'HTTPS'; Coverage = 'Company 1 HTTPS reachability plus HTTP compatibility response checks'; Mode = 'Automated' }
        ...
        [pscustomobject]@{ Block = '2'; Hotseat = '2'; Area = 'HTTPS'; Coverage = 'Company 2 HTTPS reachability plus HTTP compatibility response checks'; Mode = 'Automated' }
        ...
        [pscustomobject]@{ Block = '4'; Hotseat = '2'; Area = 'Replicated File Server'; Coverage = 'Company 2 Gluster, Samba public/private share definitions, per-user client share mounts with write-capability checks, share readability, and private-share isolation'; Mode = 'Automated' }
...
function New-LinuxPerUserMountCommands {
    ...
    foreach ($user in $Users) {
        ...
        foreach ($shareType in @('Public', 'Private')) {
            ...
            $commands += "findmnt $path >/dev/null 2>&1 && printf `"%s\n`" `"Cisco123!`" | sudo -S -p `"`" -u $user ls $path >/dev/null 2>&1 && echo ${token}_${shareToken}_SELF_OK || echo ${token}_${shareToken}_SELF_FAIL"
            $commands += "findmnt $path >/dev/null 2>&1 && printf `"%s\n`" `"Cisco123!`" | sudo -S -p `"`" -u $user test -w $path && echo ${token}_${shareToken}_WRITE_OK || echo ${token}_${shareToken}_WRITE_FAIL"
        }
    }
...
function Invoke-SB2-Hotseat1Tests {
    ...
    Invoke-ToolkitHttpTest -Name 'C1 web application HTTP compatibility response' -Uri 'http://172.30.64.162/' -Device 'C1-WebServer' -DocSection 'Service Block 2 / HTTPS' -Method 'HTTP GET' -CheckDescription 'Request the Company 1 web page over HTTP and confirm compatibility remains available after enabling HTTPS.' -CommandLines @("Invoke-WebRequest -Uri 'http://172.30.64.162/'")
}
...
function Invoke-SB2-Hotseat2Tests {
    ...
    Invoke-ToolkitHttpTest -Name 'C2 web application HTTPS response' -Uri 'https://172.30.64.170/' -Device 'C2-WebServer' -DocSection 'Service Block 2 / HTTPS' -Method 'HTTPS GET' -CheckDescription 'Request the Company 2 web page over HTTPS and confirm a valid response is returned by IP while tenant DNS separation remains enforced.' -CommandLines @("Invoke-WebRequest -Uri 'https://172.30.64.170/' -SkipCertificateCheck")
    Invoke-ToolkitHttpTest -Name 'C2 web application HTTP compatibility response' -Uri 'http://172.30.64.170/' -Device 'C2-WebServer' -DocSection 'Service Block 2 / HTTPS' -Method 'HTTP GET' -CheckDescription 'Request the Company 2 web page over HTTP and confirm compatibility remains available after enabling HTTPS.' -CommandLines @("Invoke-WebRequest -Uri 'http://172.30.64.170/'")
}
...
function Invoke-SB4-Hotseat1Tests {
    ...
    ) -MissingEvidenceStatus 'FAIL' -CheckDescription 'On C1-Client2, confirm admin, employee1, and employee2 each have their own Company 1 public and private mounts with c1.local credentials in the mount options, can read them, and have write capability on the mounted directories.' -Commands @(
...
function Invoke-SB4-Hotseat2Tests {
    ...
    Invoke-ToolkitSshTest -Name 'C2-Client1 per-user share mount summary' -Host (Get-ToolkitHost 'C2Client1').Address -User (Get-ToolkitHost 'C2Client1').LinuxUser -Device 'C2-Client1' -DocSection 'Service Block 4 / Replicated File Server' -SuccessPatterns @(
        ...
        'EMPLOYEE2_PUBLIC_MOUNT_PRESENT', 'EMPLOYEE2_PUBLIC_USERNAME_OK', 'EMPLOYEE2_PUBLIC_DOMAIN_OK', 'EMPLOYEE2_PUBLIC_SELF_OK', 'EMPLOYEE2_PUBLIC_WRITE_OK',
        'EMPLOYEE2_PRIVATE_MOUNT_PRESENT', 'EMPLOYEE2_PRIVATE_USERNAME_OK', 'EMPLOYEE2_PRIVATE_DOMAIN_OK', 'EMPLOYEE2_PRIVATE_SELF_OK', 'EMPLOYEE2_PRIVATE_WRITE_OK'
    ) -MissingEvidenceStatus 'FAIL' -CheckDescription 'On C2-Client1, confirm admin, employee1, and employee2 each have their own Company 2 public and private mounts with c2.local credentials in the mount options, can read them, and have write capability on the mounted directories.' -Commands @(
