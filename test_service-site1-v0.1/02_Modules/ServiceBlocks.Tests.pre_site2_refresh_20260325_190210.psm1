...
function Invoke-SB2-Hotseat1Tests {
    ...
    Invoke-ToolkitSshTest -Name 'C1 web application non-FQDN rejection' -Host (Get-ToolkitHost 'C1Client2').Address -User (Get-ToolkitHost 'C1Client2').LinuxUser -Device 'C1-Client2' -DocSection 'Service Block 2 / HTTPS' -SuccessPatterns @('HTTPS_IP_REJECTED', 'HTTP_IP_REJECTED', 'HTTP_FQDN_REJECTED') -CheckDescription 'From C1-Client2, confirm the Company 1 website only permits https://c1-webserver.c1.local and rejects direct-IP access plus HTTP requests.' -Commands @(
        'code=$(curl -sk --connect-timeout 5 --max-time 10 -o /dev/null -w "%{http_code}" https://172.30.64.162/ || true); if [ "$code" = "403" ] || [ "$code" = "404" ] || [ "$code" = "000" ]; then echo HTTPS_IP_REJECTED; else echo HTTPS_IP_ALLOWED:$code; fi',
        'code=$(curl -s --connect-timeout 5 --max-time 10 -o /dev/null -w "%{http_code}" http://172.30.64.162/ || true); if [ "$code" = "403" ] || [ "$code" = "404" ] || [ "$code" = "000" ]; then echo HTTP_IP_REJECTED; else echo HTTP_IP_ALLOWED:$code; fi',
        'code=$(curl -s --connect-timeout 5 --max-time 10 -o /dev/null -w "%{http_code}" http://c1-webserver.c1.local/ || true); if [ "$code" = "403" ] || [ "$code" = "404" ] || [ "$code" = "000" ]; then echo HTTP_FQDN_REJECTED; else echo HTTP_FQDN_ALLOWED:$code; fi'
    )
}
