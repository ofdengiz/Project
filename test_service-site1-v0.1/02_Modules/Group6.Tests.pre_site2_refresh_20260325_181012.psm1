...
function New-Group6Site2C2SmbPublicWriteWorkflowPlan {
    ...
    foreach ($spec in $specs) {
        ...
        $commands += (@"
printf "%s\n" "$content" > '$localFile' && out=$(smbclient //c2fs.c2.local/C2_Public -W C2 -U $($spec.MountUsername)%Cisco123! -c "put $localFile $remoteFile; ls $remoteFile" 2>&1); status=`$?; echo "`$out"; [ `$status -eq 0 ] && echo "`$out" | grep -Fq "$remoteFile" && echo ${token}_PUBLIC_CREATE_OK || echo ${token}_PUBLIC_CREATE_FAIL
"@).Trim()
    ...
    foreach ($owner in $specs) {
        ...
        foreach ($viewer in $specs | Where-Object { $_.Token -ne $ownerToken }) {
            ...
            $commands += (@"
out=$(smbclient //c2fs.c2.local/C2_Public -W C2 -U $($viewer.MountUsername)%Cisco123! -c "ls $remoteFile" 2>&1 || true); echo "`$out"; echo "`$out" | grep -Fq "$remoteFile" && echo $visibleMarker || echo $hiddenMarker
"@).Trim()
    ...
    foreach ($spec in $specs) {
        ...
        $commands += (@"
out=$(smbclient //c2fs.c2.local/C2_Public -W C2 -U $($spec.MountUsername)%Cisco123! -c "del $remoteFile" 2>&1); status=`$?; echo "`$out"; rm -f '$localFile'; [ `$status -eq 0 ] && echo ${token}_PUBLIC_CLEANUP_OK || echo ${token}_PUBLIC_CLEANUP_FAIL
"@).Trim()
...
function New-Group6Site2C2SmbPrivateWriteWorkflowPlan {
    ...
    foreach ($spec in $specs) {
        ...
        $commands += (@"
printf "%s\n" "$content" > '$localFile' && out=$(smbclient //c2fs.c2.local/C2_Private -W C2 -U $($spec.MountUsername)%Cisco123! -c "put $localFile $remoteFile; ls $remoteFile" 2>&1); status=`$?; echo "`$out"; [ `$status -eq 0 ] && echo "`$out" | grep -Fq "$remoteFile" && echo ${token}_PRIVATE_CREATE_OK || echo ${token}_PRIVATE_CREATE_FAIL
"@).Trim()
    ...
    foreach ($owner in $specs) {
        ...
        foreach ($viewer in $specs | Where-Object { $_.Token -ne $ownerToken }) {
            ...
            $commands += (@"
out=$(smbclient //c2fs.c2.local/C2_Private -W C2 -U $($viewer.MountUsername)%Cisco123! -c "ls $remoteFile" 2>&1 || true); echo "`$out"; echo "`$out" | grep -Fq "$remoteFile" && echo $visibleMarker || echo $hiddenMarker
"@).Trim()
    ...
    foreach ($spec in $specs) {
        ...
        $commands += (@"
out=$(smbclient //c2fs.c2.local/C2_Private -W C2 -U $($spec.MountUsername)%Cisco123! -c "del $remoteFile" 2>&1); status=`$?; echo "`$out"; rm -f '$localFile'; [ `$status -eq 0 ] && echo ${token}_PRIVATE_CLEANUP_OK || echo ${token}_PRIVATE_CLEANUP_FAIL
"@).Trim()
...
function Invoke-Group6SB2Hotseat1Site2Tests {
    ...
    Invoke-ToolkitSshTest -Name 'Site 2 C1UbuntuClient company-web hostname-only enforcement' -Host $c1UbuntuClient.Address -User $c1UbuntuClient.LinuxUser -Device $c1UbuntuClient.DisplayName -DocSection 'Service Block 2 / Hotseat 1 / Site 2 Company 1 client' -Commands @(
        ...
        'code=$(curl -k -s --connect-timeout 5 --max-time 10 -o /dev/null -w "%{http_code}" https://172.30.65.162/ 2>/dev/null || true); if [ "$code" = "403" ] || [ "$code" = "404" ] || [ "$code" = "000" ]; then echo S2_C1CLIENT_C1WEB_HTTPS_IP_HARDENED; else echo S2_C1CLIENT_C1WEB_HTTPS_IP_ALLOWED:$code; fi',
        'code=$(curl -s --connect-timeout 5 --max-time 10 -o /dev/null -w "%{http_code}" http://172.30.65.162/ 2>/dev/null || true); if [ "$code" = "403" ] || [ "$code" = "404" ] || [ "$code" = "000" ]; then echo S2_C1CLIENT_C1WEB_HTTP_IP_HARDENED; else echo S2_C1CLIENT_C1WEB_HTTP_IP_ALLOWED:$code; fi',
        'code=$(curl -s --connect-timeout 5 --max-time 10 -o /dev/null -w "%{http_code}" http://c1-webserver.c1.local/ 2>/dev/null || true); if [ "$code" = "403" ] || [ "$code" = "404" ] || [ "$code" = "000" ]; then echo S2_C1CLIENT_C1WEB_HTTP_FQDN_HARDENED; else echo S2_C1CLIENT_C1WEB_HTTP_FQDN_ALLOWED:$code; fi'
    ) -DisplayCommands @(
