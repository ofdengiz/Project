function Get-Group6Coverage {
    ...
    @(
        ...
        [pscustomobject]@{ Block = '4'; Hotseat = '1'; Area = 'Company 1 file services'; Coverage = 'Site 1 DFS/iSCSI/share proof plus namespace-target redundancy and Site 2 Company 1 file-path / SAN isolation validation.'; Mode = 'Automated + manual adjunct' }
        [pscustomobject]@{ Block = '4'; Hotseat = '2'; Area = 'Company 2 file services'; Coverage = 'Site 1 mounted-share proof plus Site 2 Company 2 storage-chain, hostname-based SMB access, and per-user private-view evidence.'; Mode = 'Automated' }
...
function New-Group6Site2C2SmbBrowseCommands {
    @(
        ...
        'browse_out=$(smbclient -g -L //c2fs.c2.local -W C2 -U employee1%Cisco123! 2>&1 || true); echo "$browse_out"; echo "$browse_out" | grep -Fq "Disk|C2_Public|" && echo C2FS_BROWSE_PUBLIC_OK || echo C2FS_BROWSE_PUBLIC_FAIL; echo "$browse_out" | grep -Fq "Disk|C2_Private|" && echo C2FS_BROWSE_PRIVATE_OK || echo C2FS_BROWSE_PRIVATE_FAIL'
    )
}
...
function New-Group6Site2C2SmbShareCommands {
    ...
    foreach ($spec in Resolve-Group6LinuxUserSpecs -UserSpecs $UserSpecs) {
        ...
        foreach ($shareType in $ShareTypes) {
            ...
            $commands += ('out=$(smbclient //c2fs.c2.local/{0} -W C2 -U {1}%Cisco123! -c ''ls'' 2>&1 || true); echo "$out"; echo "$out" | grep -q ''blocks of size'' && echo {2}_{3}_ACCESS_OK || echo {2}_{3}_ACCESS_FAIL' -f $shareName, $spec.MountUsername, $token, $shareToken)
        }
    }
...
function Invoke-Group6SB2Hotseat1AdditionalTests {
    ...
    Invoke-ToolkitSshTest -Name 'C1-Client2 hostname-only company-web validation' -Host $c1Client2.Address -User $c1Client2.LinuxUser -Device $c1Client2.DisplayName -DocSection 'Service Block 2 / HTTPS' -Commands @(
        ...
        'code=$(curl -sk --connect-timeout 5 --max-time 10 -o /dev/null -w "%{http_code}" https://172.30.64.162/ 2>/dev/null || true); if [ "$code" = "403" ] || [ "$code" = "404" ] || [ "$code" = "000" ]; then echo C1CLIENT_HTTPS_IP_HARDENED; else echo C1CLIENT_HTTPS_IP_ALLOWED:$code; fi',
        'code=$(curl -s --connect-timeout 5 --max-time 10 -o /dev/null -w "%{http_code}" http://172.30.64.162/ 2>/dev/null || true); if [ "$code" = "403" ] || [ "$code" = "404" ] || [ "$code" = "000" ]; then echo C1CLIENT_HTTP_IP_HARDENED; else echo C1CLIENT_HTTP_IP_ALLOWED:$code; fi',
        'code=$(curl -s --connect-timeout 5 --max-time 10 -o /dev/null -w "%{http_code}" http://c1-webserver.c1.local/ 2>/dev/null || true); if [ "$code" = "403" ] || [ "$code" = "404" ] || [ "$code" = "000" ]; then echo C1CLIENT_HTTP_FQDN_HARDENED; else echo C1CLIENT_HTTP_FQDN_ALLOWED:$code; fi'
    ) -DisplayCommands @(
...
function Invoke-Group6SB2Hotseat1Site2Tests {
    ...
    Invoke-ToolkitSshTest -Name 'Site 2 Company 1 web hostname-only access summary' -Host $mspJump.Address -User $mspJump.LinuxUser -Device 'MSPUbuntuJump -> Site 2 C1 web' -DocSection 'Service Block 2 / Hotseat 1 / Site 2 Company 1 web' -Commands @(
        'code=$(curl -k -s --connect-timeout 5 --max-time 10 -o /dev/null -w "%{http_code}" https://c1-webserver.c1.local/ 2>/dev/null || true); if [ "$code" = "200" ]; then echo S2_C1WEB_FQDN_OK; else echo S2_C1WEB_FQDN_FAIL:$code; fi',
        'code=$(curl -k -s --connect-timeout 5 --max-time 10 -o /dev/null -w "%{http_code}" https://172.30.65.162/ 2>/dev/null || true); if [ "$code" = "403" ] || [ "$code" = "404" ] || [ "$code" = "000" ]; then echo S2_C1WEB_HTTPS_IP_HARDENED; else echo S2_C1WEB_HTTPS_IP_ALLOWED:$code; fi',
        'code=$(curl -s --connect-timeout 5 --max-time 10 -o /dev/null -w "%{http_code}" http://172.30.65.162/ 2>/dev/null || true); if [ "$code" = "403" ] || [ "$code" = "404" ] || [ "$code" = "000" ]; then echo S2_C1WEB_HTTP_IP_HARDENED; else echo S2_C1WEB_HTTP_IP_ALLOWED:$code; fi',
        'code=$(curl -s --connect-timeout 5 --max-time 10 -o /dev/null -w "%{http_code}" http://c1-webserver.c1.local/ 2>/dev/null || true); if [ "$code" = "403" ] || [ "$code" = "404" ] || [ "$code" = "000" ]; then echo S2_C1WEB_HTTP_FQDN_HARDENED; else echo S2_C1WEB_HTTP_FQDN_ALLOWED:$code; fi'
    ) -DisplayCommands @(
        ...
        'curl -s --connect-timeout 5 --max-time 10 -o /dev/null -w "%{http_code}" http://c1-webserver.c1.local/'
    ) -SuccessPatterns @('S2_C1WEB_FQDN_OK', 'S2_C1WEB_HTTPS_IP_HARDENED', 'S2_C1WEB_HTTP_IP_HARDENED', 'S2_C1WEB_HTTP_FQDN_HARDENED') -MissingEvidenceStatus 'FAIL' -CommandTimeoutSeconds 60 -CheckDescription 'From Site 2 MSPUbuntuJump, confirm Company 1 web access succeeds only through the HTTPS hostname while direct-IP and HTTP access remain hardened.'
}
...
function Invoke-Group6SB2Hotseat2AdditionalTests {
    ...
    Invoke-ToolkitSshTest -Name 'Site 2 C2LinuxClient company-web hostname-only enforcement' -Host $c2LinuxClient.Address -User $c2LinuxClient.LinuxUser -Device $c2LinuxClient.DisplayName -DocSection 'Service Block 2 / Hotseat 2 / Site 2 Company 2 client' -Commands @(
        ...
        'code=$(curl -k -s --connect-timeout 5 --max-time 10 -o /dev/null -w "%{http_code}" https://172.30.65.170/ 2>/dev/null || true); if [ "$code" = "403" ] || [ "$code" = "404" ] || [ "$code" = "000" ]; then echo S2_C2CLIENT_C2WEB_HTTPS_IP_HARDENED; else echo S2_C2CLIENT_C2WEB_HTTPS_IP_ALLOWED:$code; fi',
        'code=$(curl -s --connect-timeout 5 --max-time 10 -o /dev/null -w "%{http_code}" http://172.30.65.170/ 2>/dev/null || true); if [ "$code" = "403" ] || [ "$code" = "404" ] || [ "$code" = "000" ]; then echo S2_C2CLIENT_C2WEB_HTTP_IP_HARDENED; else echo S2_C2CLIENT_C2WEB_HTTP_IP_ALLOWED:$code; fi',
        'code=$(curl -s --connect-timeout 5 --max-time 10 -o /dev/null -w "%{http_code}" http://c2-webserver.c2.local/ 2>/dev/null || true); if [ "$code" = "403" ] || [ "$code" = "404" ] || [ "$code" = "000" ]; then echo S2_C2CLIENT_C2WEB_HTTP_FQDN_HARDENED; else echo S2_C2CLIENT_C2WEB_HTTP_FQDN_ALLOWED:$code; fi'
    ) -DisplayCommands @(
...
function Invoke-Group6SB2Hotseat2Site2Tests {
    ...
    Invoke-ToolkitSshTest -Name 'Site 2 Company 2 web hostname-only access summary' -Host $mspJump.Address -User $mspJump.LinuxUser -Device 'MSPUbuntuJump -> Site 2 C2 web' -DocSection 'Service Block 2 / Hotseat 2 / Site 2 Company 2 web' -Commands @(
        'code=$(curl -k -s --connect-timeout 5 --max-time 10 -o /dev/null -w "%{http_code}" https://c2-webserver.c2.local/ 2>/dev/null || true); if [ "$code" = "200" ]; then echo S2_C2WEB_FQDN_OK; else echo S2_C2WEB_FQDN_FAIL:$code; fi',
        'code=$(curl -k -s --connect-timeout 5 --max-time 10 -o /dev/null -w "%{http_code}" https://172.30.65.170/ 2>/dev/null || true); if [ "$code" = "403" ] || [ "$code" = "404" ] || [ "$code" = "000" ]; then echo S2_C2WEB_HTTPS_IP_HARDENED; else echo S2_C2WEB_HTTPS_IP_ALLOWED:$code; fi',
        'code=$(curl -s --connect-timeout 5 --max-time 10 -o /dev/null -w "%{http_code}" http://172.30.65.170/ 2>/dev/null || true); if [ "$code" = "403" ] || [ "$code" = "404" ] || [ "$code" = "000" ]; then echo S2_C2WEB_HTTP_IP_HARDENED; else echo S2_C2WEB_HTTP_IP_ALLOWED:$code; fi',
        'code=$(curl -s --connect-timeout 5 --max-time 10 -o /dev/null -w "%{http_code}" http://c2-webserver.c2.local/ 2>/dev/null || true); if [ "$code" = "403" ] || [ "$code" = "404" ] || [ "$code" = "000" ]; then echo S2_C2WEB_HTTP_FQDN_HARDENED; else echo S2_C2WEB_HTTP_FQDN_ALLOWED:$code; fi'
    ) -DisplayCommands @(
        'curl -k -s --connect-timeout 5 --max-time 10 -o /dev/null -w "%{http_code}" https://c2-webserver.c2.local/',
        'curl -k -s --connect-timeout 5 --max-time 10 -o /dev/null -w "%{http_code}" https://172.30.65.170/',
        'curl -s --connect-timeout 5 --max-time 10 -o /dev/null -w "%{http_code}" http://172.30.65.170/',
        'curl -s --connect-timeout 5 --max-time 10 -o /dev/null -w "%{http_code}" http://c2-webserver.c2.local/'
    ) -SuccessPatterns @('S2_C2WEB_FQDN_OK', 'S2_C2WEB_HTTPS_IP_HARDENED', 'S2_C2WEB_HTTP_IP_HARDENED', 'S2_C2WEB_HTTP_FQDN_HARDENED') -MissingEvidenceStatus 'FAIL' -CommandTimeoutSeconds 60 -CheckDescription 'From Site 2 MSPUbuntuJump, confirm Company 2 web access succeeds via FQDN and direct-IP or HTTP access remains hardened.'
...
function Invoke-Group6SB4Hotseat1AdditionalTests {
    ...
    Invoke-ToolkitWinrmTest -Name 'C1 DFS namespace target-set summary' -ComputerName $c1dc1.Address -Device $c1dc1.DisplayName -DocSection 'Service Block 4 / Replicated File Server' -CommandLines @(
        "Get-DfsnFolderTarget -Path '\\c1.local\\namespace\\public'",
        "Get-DfsnFolderTarget -Path '\\c1.local\\namespace\\private'"
    ) -ScriptBlock {
        $publicTargets = @(Get-DfsnFolderTarget -Path '\\c1.local\namespace\public' | Select-Object -ExpandProperty TargetPath)
        $privateTargets = @(Get-DfsnFolderTarget -Path '\\c1.local\namespace\private' | Select-Object -ExpandProperty TargetPath)

        $publicText = $publicTargets -join "`n"
        $privateText = $privateTargets -join "`n"

        if ($publicText -match 'C1-DC1' -and $publicText -match 'C1-DC2' -and $publicText -match 'C1FS') {
        ...
        if ($privateText -match 'C1-DC1' -and $privateText -match 'C1-DC2' -and $privateText -match 'C1FS') {
        ...
        $publicTargets
        $privateTargets
    } -SuccessPatterns @('PUBLIC_TARGET_SET_OK', 'PRIVATE_TARGET_SET_OK') -CheckDescription 'On C1-DC1, confirm the DFS namespace still publishes redundant public and private folder targets across C1-DC1, C1-DC2, and C1FS so namespace failover remains available.'
    ...
    } -SuccessPatterns @('S2_C1FS_SHARES_OK', 'S2_C1FS_ISCSI_OK', 'S2_C1FS_STORAGE_OK') -CheckDescription 'On Site 2 C1FS, confirm the file server is publishing SMB shares from attached storage rather than existing only as a reachable endpoint.'
}
...
function Invoke-Group6SB4Hotseat2Site2Tests {
    ...
    Invoke-ToolkitSshTest -Name 'Site 2 C2LinuxClient hostname-based SMB browse summary' -Host $c2LinuxClient.Address -User $c2LinuxClient.LinuxUser -Device $c2LinuxClient.DisplayName -DocSection 'Service Block 4 / Hotseat 2 / Site 2 Company 2 client' -Commands (New-Group6Site2C2SmbBrowseCommands) -SuccessPatterns @(
        'SMBCLIENT_PRESENT','C2FS_HOST_OK','C2FS_BROWSE_PUBLIC_OK','C2FS_BROWSE_PRIVATE_OK'
    ) -MissingEvidenceStatus 'FAIL' -CommandTimeoutSeconds 60 -CheckDescription 'On Site 2 C2LinuxClient, confirm smbclient is available, c2fs.c2.local resolves by name, and authenticated hostname-based SMB browse exposes C2_Public plus C2_Private.'
    ...
    $employeePrivateCommands = New-Group6Site2C2SmbShareCommands -UserSpecs @($c2Specs | Where-Object { $_.Token -in @('EMPLOYEE1', 'EMPLOYEE2') }) -ShareTypes @('Private')
    Invoke-ToolkitSshTest -Name 'Site 2 C2LinuxClient private share per-user-view summary' -Host $c2LinuxClient.Address -User $c2LinuxClient.LinuxUser -Device $c2LinuxClient.DisplayName -DocSection 'Service Block 4 / Hotseat 2 / Site 2 Company 2 client' -Commands $employeePrivateCommands -SuccessPatterns @(
        'EMPLOYEE1_PRIVATE_ACCESS_OK','EMPLOYEE2_PRIVATE_ACCESS_OK'
    ) -MissingEvidenceStatus 'FAIL' -CommandTimeoutSeconds 60 -CheckDescription 'On Site 2 C2LinuxClient, confirm employee1@c2.local and employee2@c2.local each reach C2_Private through their own authenticated SMB view; the server-side %U isolation mapping is validated separately on C2FS.'
}
